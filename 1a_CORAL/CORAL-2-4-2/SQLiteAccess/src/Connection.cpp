...
coral::SQLiteAccess::Connection::verifyFileName( const std::string& inFileName ) const
{
  ...
  if( thePathVariable )
  {
    ...
    // Workaround for Coverity UNREACHABLE (bug #95671 on sr #101004)
    // The workaround will silence Coverity but the functional bug is pending!
    const boost::filesystem::path guess( *(path.begin())/fileNameFirstGuess );
