...
#define LOG( msg ){ std::cout << "__ObjectConsistencyTest " << msg << std::endl; }
...
public:
  ...
  public:
    ...
    {
      coral::lock_guard lock( s_mutex ); // added by AV May 2012...
      if ( !s_session ) s_session=session;
      m_ithread = s_nthreads++; // 0-19
      //LOG("Creating thread #" << m_ithread << " " << this);
    }

    ObjectConsistencyThread( const ObjectConsistencyThread& rhs )
      : CoralCppUnitDBTest( rhs )
    {
      m_ithread = rhs.m_ithread;
      //LOG("Copy-creating thread #" << m_ithread << " " << this);
    }

    ~ObjectConsistencyThread()
    {
      //LOG("Deleting thread #" << m_ithread << " " << this);
    }

    void operator()()
    {
      LOG("testThreads ENTER thread #" << m_ithread);
      std::string T1 = BuildUniqueTableName( "OBJCONSIST_T1" );
      coral::AttributeList rowBuffer;
      rowBuffer.extend( "id", typeid(unsigned long long));
      rowBuffer.extend( "content", typeid(std::string) );
      ...
      try
      {
        std::auto_ptr<coral::IQuery> query; // fix memory leak (bug #98736)
        {
          coral::lock_guard lock( s_mutex );
          if ( !s_session )
          {
            errorsInThreads( true );
            return;
          }
          LOG("testThreads GET SCHEMA thread #" << m_ithread);
          coral::ISchema& schema = s_session->nominalSchema();
          LOG("testThreads GOT SCHEMA thread #" << m_ithread);
          coral::ITable& table = schema.tableHandle(T1);
          query.reset( table.newQuery() );
        }
        query->addToOrderList( "id" );
        query->addToOutputList( "id" );
        query->addToOutputList( "content" );
        ...
        LOG("testThreads EXECUTE QUERY " << query.get() << " thread #" << m_ithread);
        coral::ICursor& cursor = query->execute();
        // If everything is ok continue
        sleep( 1 );
        // Now the session is for sure deleted
        LOG("testThreads START CURSOR LOOP thread #" << m_ithread);
        while(cursor.next())
        {
          LOG("testThreads CURSOR LOOPING thread #" << m_ithread);
        }
      ...
      {
        LOG("testThreads EXCEPTION thread #" << m_ithread << " : " << e.what() );
      }
      LOG("testThreads LEAVE thread #" << m_ithread);
    }
    ...
    {
      coral::lock_guard lock( s_mutex );
      LOG("testThreads (main) ** DELETE SESSION");
      if ( s_session ) delete s_session;
      s_session = 0;
      LOG("testThreads (main) ** DELETED SESSION");
    }

    static coral::mutex s_mutex;
  ...
  private:
    ...
    ObjectConsistencyThread& operator=( const ObjectConsistencyThread& rhs );
  ...
  void test_threads()
  {
    ...
    // open a read only transaction
    LOG("testThreads (main) ** START TX");
    session->transaction().start(true);
    LOG("testThreads (main) ** STARTED TX");
...
#define __THREADS 20
    ...
    {
      threads.push_back( new boost::thread( ObjectConsistencyThread( session ) ) );
    }

    sleep(1);
    LOG("testThreads (main) ** CALL DELETESESSION");
    ObjectConsistencyThread::deleteSession();
    LOG("testThreads (main) ** CALLED DELETESESSION");

    // join all threads
    for( size_t i = 0; i < __THREADS; ++i )
    {
      LOG("testThreads (main) ** JOIN THREAD #" << i << " **" );
      threads[i]->join();
      LOG("testThreads (main) ** JOINED THREAD #" << i << " **" );
    }

    // clean up threads
    for( size_t i = 0; i < __THREADS; ++i )
    {
      LOG("testThreads (main) ** DELETE THREAD #" << i << " **" );
      delete threads[i];
      LOG("testThreads (main) ** DELETED THREAD #" << i << " **" );
    }

    if ( ObjectConsistencyThread::errorsInThreads() )
    {
      LOG( "testThreads (main) ** ERROR IN "
...
coral::mutex coral::ObjectConsistencyTest::ObjectConsistencyThread::s_mutex;

coral::ISessionProxy* coral::ObjectConsistencyTest::ObjectConsistencyThread::s_session = 0;

unsigned coral::ObjectConsistencyTest::ObjectConsistencyThread::s_nthreads = 0;
