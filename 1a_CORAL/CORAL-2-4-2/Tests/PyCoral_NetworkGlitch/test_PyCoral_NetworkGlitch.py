...
class TestNetworkGlitch( PyCoralTest.TestCase ):
    ...
    def test136a_IConnectionToCursorWithThreeSessions_connSharing(self):
        ...
        except Exception,error: print "ERROR!",error
    ...
    def internalTest136_IConnectionToCursorWithThreeSessions(self):
        ...
        self.assertEquals( session1.transaction().isReadOnly(), True )
        if dbg: print "SCHEMA2 SESSION2"
        schema2 = session2.nominalSchema()
        self.assertEquals( session2.transaction().isReadOnly(), True )
...
if __name__ == '__main__':
    ...
    # NB The test2_nightly service uses only one instance: this used to be
    # test22 but recently moved to test21, causing issues (bug #102664)
    ###tnsoutput = tnsalias+"=(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=localhost)(PORT="+str(lport)+")))(CONNECT_DATA=(SERVICE_NAME=test2_nightly.cern.ch)(INSTANCE_NAME=test21)(SERVER= DEDICATED)))"
    # You may use the test2.cern.ch service (defined on both nodes) to debug
    ###tnsoutput = tnsalias+"=(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=localhost)(PORT="+str(lport)+")))(CONNECT_DATA=(SERVICE_NAME=test2.cern.ch)(INSTANCE_NAME=test21)(SERVER= DEDICATED)))"
    # NB The test2_nightly service uses only one instance: this should always
    # be test21 but can be test22 if DBAs forget to 'relocate' it (bug #103395)
    # To improve reliability, use the test2_lb service (defined on both nodes):
    # as INSTANCE_NAME is specified in tnsnames.ora, connections will not move
    tnsoutput = tnsalias+"=(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=localhost)(PORT="+str(lport)+")))(CONNECT_DATA=(SERVICE_NAME=test2_lb.cern.ch)(INSTANCE_NAME=test21)(SERVER= DEDICATED)))"    
    fd = open(tnsfile, 'w')
    fd.write(tnsoutput)
    fd.close()
