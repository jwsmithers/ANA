...
# To debug, try 'sqlplus avalassi/xxx@test21-v:10121/test2_nightly.cern.ch'
# Alternatively, try test22 as the service may have moved from one node to the
# other (see bug #102664) - note that test.cern.ch is defined on both nodes
ghost = socket.gethostname()
# Note also that test2x-s moves connections to test2x-v, which is not good for
# the network glitch test as breaking the tunnel does not break the connection
# Use netstat to check where the listener opens the connection socket
###tport = "test21-s.cern.ch:10121" # not OK (socket opened on test21-v)
tport = "test21-v.cern.ch:10121" # OK (socket opened on test21-v)
###tport = "test21.cern.ch:10121" # also OK (socket opened on test21)
