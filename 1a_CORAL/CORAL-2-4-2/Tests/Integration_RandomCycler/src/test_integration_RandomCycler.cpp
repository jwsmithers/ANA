...
public:
  ...
  void test_r02(std::auto_ptr<coral::ISessionProxy>& session)
  {
    ...
    while(cursor.next())
    {
      ...
      // compare the content
      CPPUNIT_ASSERT_EQUAL_MESSAGE( "value missmatch in T1 in column 'id'", row["id"].data<unsigned long long>(), counter );

      CPPUNIT_ASSERT_EQUAL_MESSAGE( "value missmatch in T1 in column 'content'", row["content"].data<std::string>().compare("hello world"), 0 );
    }
    session->transaction().commit();
  ...
  void operator()()
  {
    ...
    coral::boost_random_uniform_int rn5(0,4);
    coral::boost_random_uniform_int rn3(0,2);
    for( size_t i = 0; i < 10; ++i )
    {
      coral::sleepSeconds( rn5(rng) ); // Fix Coverity SECURE_CODING (replace rand)
      // get a random test
      size_t rnumber = ( rn3(rng) ); // Fix Coverity SECURE_CODING (replace rand)
      switch(rnumber)
      {
      case 0:
        test_r01( session );
