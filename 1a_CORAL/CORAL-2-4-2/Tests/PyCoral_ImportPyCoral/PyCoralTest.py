...
class TestCase( unittest.TestCase ):
    ...
    def __str__(self):
        ...
        sep = self.separator("+")
        if "LCG_NGT_SLT_NAME" in os.environ and "CMTCONFIG" in os.environ: sep = "%s\n[%s on %s slot]"%(sep, os.environ["CMTCONFIG"], os.environ["LCG_NGT_SLT_NAME"])
...
def buildUniqueTableName( name ):
    ...
    platformPrefix = ""
    if "CORAL_CMTCONFIG_HASH" in os.environ:
        platformPrefix = os.environ["CORAL_CMTCONFIG_HASH"]
        if len(platformPrefix) != 3 :
            print "ERROR! CORAL_CMTCONFIG_HASH '"+platformPrefix+"' is not three-character long"
            exit(1)
        elif platformPrefix == '999' :
            print "ERROR! No platform-specific CORAL_CMTCONFIG_HASH is defined: add CMTCONFIG to cmt/requirements"
            exit(1)
    else:
        print "WARNING! CORAL_CMTCONFIG_HASH is not set"
...
def buildUrl( backend, readOnly ):
    if backend != "SQLite" :
        if readOnly:
            return "CORAL-" + backend + "-sftnight/reader"
        else:
            return "CORAL-" + backend + "-sftnight/admin"
    else:
