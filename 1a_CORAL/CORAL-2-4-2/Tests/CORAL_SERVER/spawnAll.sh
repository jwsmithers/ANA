...
if [ $startServer == 1 ]; then 
  echo "==> EXECUTE ./startServer.sh $test $cout2 $lcg &"
  ./startServer.sh $test $cout2 $lcg &
fi
if [ $startProxy == 1 ]; then
  echo "==> EXECUTE ./startProxy.sh $test $cout2 $lcg &"
  ./startProxy.sh $test $cout2 $lcg &
fi
sleep 1s # Allow time to kill all running coralServer and coralServerProxy
echo "==> EXECUTE ./allTests.sh $wrap $test $conn $cout1 $lcg"
./allTests.sh $wrap $test $conn $cout1 $lcg
echo "Client: returned control"

if [ $startProxy == 1 ]; then
  echo "Proxy : kill it using SIGTERM"
  killall coralServerProxy >& /dev/null
fi

if [ $startServer == 1 ]; then
  echo "Server: kill it using SIGTERM"
  kill -TERM `ps -ef | grep $USER | grep valgrind | grep coralServer | grep -v /usr/bin/time | grep -v grep | awk '{print $2}'` >& /dev/null
  killall coralServer >& /dev/null # Send SIGTERM: server will cleanly shutdown within ~2s
