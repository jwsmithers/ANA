...
#define ERROR(out) do { coral::MessageStream log("coral_authentication_manager"); log << coral::Error << out << coral::MessageStream::endmsg; } while (0)
...
void delPermission( ISchema& schema, CommandLine & userOptions )
{
  ...
  DEBUG("Removed " << count <<" permissions.");
}
