...
#define DEBUG( msg ) { coral::MessageStream myMsg("CoralAuthenticationService"); myMsg << coral::Debug << msg << coral::MessageStream::endmsg; }
                                        ...
                                        const std::string& user, const std::string& dbRole )
{
  ...
  while ( cursor.next() ) {
    ...
    DEBUG( " connection: '" << curr[ columnNames::physConStr() ].data<
