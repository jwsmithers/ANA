...
#define DEBUG( msg ) { coral::MessageStream myMsg("CoralAuthenticationService::QueryMgr"); myMsg << coral::Debug << msg << coral::MessageStream::endmsg; }
...
void
QueryMgr::addConnection( const std::string& lcString,
                         ...
                         const std::string& authMechanism )
{
  ...
  if ( !accessMode.empty() )
  {
    if ( accessMode == "update" ||
      ...
      throw coral::Exception( "access mode should either be 'update' or 'read-only'",
...
void
QueryMgr::delConnection( const std::string& lcString,
                         const std::string& pcString,
                         const std::string& user,
                         const std::string& role_ )
{
  ...
  if ( !user.empty() || !role.empty() ) {
    ...
    *///
    ...
    DEBUG( "Deleted "<< deleted << " credentials" );
  };
                                ...
                                const std::string& user, const std::string& dbRole )
{
  ...
  while ( cursor.next() ) {
    const AttributeList &curr( cursor.currentRow() );
    //DEBUG(" connection: '" << curr );
    //std::ios_base::fmtflags original_flags = std::cout.flags(); // Fix Coverity STREAM_FORMAT_STATE
    DEBUG( std::setw(10) << curr[ "lcs" ].data< std::string >() << "  "
           << std::setw(30) << curr[ "physConStr" ].data< std::string >() << "  "
           << std::setw(10) << curr[ "user" ].data< std::string >() << "  "
           << std::setw(10) << curr[ "role" ].data< std::string >() << "  "
           << std::setw(30) << curr[ "fqan" ].data< std::string >() << "  "
           << std::setw(30) << curr[ "dn" ].data< std::string >() << "  " );
    //std::cout.flags(original_flags); // Fix Coverity STREAM_FORMAT_STATE
    /*[ columnNames::physConStr() ].data<
        columnTypes::physConStr>()
        << "' User: '" << curr[ columnNames::user() ].data<
        columnTypes::user>()
  ...
  if ( cert->FQANs().size() > 0 ) {
    ...
    while ( it != cert->FQANs().end() )
    {
      ...
      whereData.extend( name2.str(),
                        AttributeSpecification::typeNameForType<PermissionsTable::columnTypes::vomsFQAN>() );
