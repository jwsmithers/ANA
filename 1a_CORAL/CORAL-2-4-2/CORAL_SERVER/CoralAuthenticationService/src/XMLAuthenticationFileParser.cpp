// Disable warnings triggered by the XercesC headers on icc (bug #100415)
#if defined __ICC
...
bool
coral::XMLAuthenticationFileParser::parse( const std::string& inputFileName,
    ...
    if ( connectionList )
    {
      ...
      for ( XMLSize_t iConnection = 0; iConnection < numberOfConnections; ++iConnection )
      {
        ...
        if ( connectionNode )
        {
          ...
          std::map< std::string, coral::AuthenticationCredentialSet* >::iterator iMyConnection = data.find( sConnectionName );
          if ( iMyConnection != data.end() )
          {
            credential = iMyConnection->second;
