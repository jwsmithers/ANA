// $Id: coralDummySocketClient.cpp,v 1.1.2.6.2.2 2013-02-08 09:40:14 avalassi Exp $
...
namespace coral
{
  namespace CoralSockets
  {
    ...
    std::auto_ptr<ByteBuffer> copyByteBuffer( const ByteBuffer& orig )
    {
      std::auto_ptr<ByteBuffer> ret( new ByteBuffer( orig.usedSize() ) );
      for (unsigned int i=0; i<orig.usedSize(); i++)
        *(ret->data()+i) = *(orig.data()+i);
    ...
    public:
      ...
      IByteBufferIteratorPtr getReply( IRequestHandler* handler, const ByteBuffer& buffer )
      {
        RequestProperties props;
        RequestIterator *it=new RequestIterator();
        it->addBuffer( copyByteBuffer( buffer ), 0, true /*isLast*/ );
        return handler->replyToRequest( IByteBufferIteratorPtr( it ), props );
      };
      ...
      void operator()()
      {
        ...
        try {
          // Test #1 - make sure the server can handle long strings
          {
            ...
            IByteBufferIteratorPtr reply( getReply( m_handler, request  ) );
            //std::cout << "__TestClientThreadMT Received reply (payload size=" << reply.size() << ")" << std::endl;
            IByteBufferIteratorPtr expReply( getReply( &dummyHandler, request ) );
          ...
          // Test #5 - simulate long reply times from the backend database
          {
            ...
            if ( !reply->nextBuffer() ) throw GenericSocketException("no reply to sleep request?");
