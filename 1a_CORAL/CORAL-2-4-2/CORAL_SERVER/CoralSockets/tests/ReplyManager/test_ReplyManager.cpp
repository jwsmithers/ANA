// $Id: test_ReplyManager.cpp,v 1.8.2.3.2.2 2013-02-08 09:40:12 avalassi Exp $
  ...
  public:
    ...
    void test_ReplyIterator()
    {
      ...
      slot.takeSlot( 1 );
      {
        ...
        // before the first call to nextBuffer(), the iterator should throw
        ...
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "1 nextBuffer", it.nextBuffer(), true );
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "1 isLastBuffer", it.isLastBuffer(), false );
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "1 currentBuffer",
                                      getBufferNum( it.currentBuffer() ), 10 );

        CPPUNIT_ASSERT_EQUAL_MESSAGE( "2 nextBuffer", it.nextBuffer(), true );
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "2 isLastBuffer", it.isLastBuffer(), false );
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "2 currentBuffer",
                                      getBufferNum( it.currentBuffer() ), 20 );


        CPPUNIT_ASSERT_EQUAL_MESSAGE( "3 nextBuffer", it.nextBuffer(), true );
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "3 isLastBuffer", it.isLastBuffer(), false );
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "3 currentBuffer",
                                      getBufferNum( it.currentBuffer() ), 30 );


        CPPUNIT_ASSERT_EQUAL_MESSAGE( "4 nextBuffer", it.nextBuffer(), true );
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "4 isLastBuffer", it.isLastBuffer(), false );
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "4 currentBuffer",
                                      getBufferNum( it.currentBuffer() ), 40 );

        CPPUNIT_ASSERT_EQUAL_MESSAGE( "5 nextBuffer", it.nextBuffer(), true );
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "5 isLastBuffer", it.isLastBuffer(), true );
        CPPUNIT_ASSERT_EQUAL_MESSAGE( "5 currentBuffer",
                                      getBufferNum( it.currentBuffer() ), 50 );

        CPPUNIT_ASSERT_EQUAL_MESSAGE( "6 nextBuffer", it.nextBuffer(), false );

        // after nextBuffer() returned false, the iterator should throw
        CPPUNIT_ASSERT_THROW( it.currentBuffer(),
                              GenericSocketException );
        CPPUNIT_ASSERT_THROW( it.isLastBuffer(),
                              GenericSocketException );
    ...
    void test_ReplyIteratorTimeOut()
    {
      ...
      CPPUNIT_ASSERT_EQUAL_MESSAGE( "1 currentBuffer",
                                    getBufferNum( it.currentBuffer() ), 10 );
      try {
        it.nextBuffer(); // this should time out
...
#define MSG_COUNT 100
    ...
    void test_ReplyManager()
    {
      ...
      {
        coral::mutex creatorMutex;

        coral::mutex resultMutex;
        bool result=true;
