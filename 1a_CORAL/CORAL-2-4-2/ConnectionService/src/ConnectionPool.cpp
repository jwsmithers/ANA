...
coral::ConnectionService::ConnectionPool::~ConnectionPool()
{
  ...
  {
    coral::lock_guard lock( m_mutexForThread );
                                                                       ...
                                                                       AccessMode accessMode )
{
  ...
  try
  {
    ...
    if ( connection )
    {
      ...
      {
        m_replicaCatalogue->setConnectionExcluded( connectionParameter.technologyName(), connectionParameter.serviceName() );
      }
    }
    else
    {
      log << coral::Debug << "Could not get a connection to service \""
          << connectionParameter.serviceName() << "\" from domain. "
          << coral::MessageStream::endmsg;
  ...
  {
    coral::lock_guard lock(m_mutexForPool);
                                                                        ...
                                                                        AccessMode accessMode )
{
  ...
  {
    ConnectionMap::Iterator iConn = m_activeConnections.lookup( connectionParameter.technologyName(), connectionParameter.serviceName() );
    ...
    if ( connected )
    {
      ...
      if ( connection.specificTimeOut()>0 )
      {
        ...
        log << coral::Debug
            ...
            << " (timeout=" << connection.specificTimeOut() << " is > 0)"
            << coral::MessageStream::endmsg;
      }
    }
    else
...
coral::ConnectionService::ConnectionPool::removeConnection( ConnectionHandle& connection )
{
  ...
  m_activeConnections.remove( connection );
  coral::MessageStream log( m_connectionServiceConfiguration.serviceName() );
  log << coral::Debug << "The invalid connection id=" << connection.connectionId()<<" to service \"" << connection.serviceName() << "\" has been removed from the pool." << coral::MessageStream::endmsg;
...
{
  coral::lock_guard lockForDestruction(m_mutexForThread);
  coral::lock_guard lockForCleanUp(m_mutexForPool);
  if( m_idleConnections.size() > 0 )
  {
    ...
    for ( std::vector< ConnectionMap::Iterator >::iterator vecIter = timedOutConnections.begin(); vecIter!= timedOutConnections.end(); vecIter++ )
    {
...
coral::ConnectionService::ConnectionPool::waitForTimeout()
{
  ...
  int poolCleanupPeriod = m_connectionServiceConfiguration.poolCleanUpPeriod();
  if ( poolCleanupPeriod <= 0 ) poolCleanupPeriod = 5;  // Default: 5 seconds
  //std::cout << svcName << " ... poolCleanup - sleep "
  //          << poolCleanUpPeriod << " seconds" << std::endl;

  // Try to lock, until timeout reached or unlocked by main thread
  if ( m_finalized.timed_lock( boost::posix_time::milliseconds( poolCleanupPeriod*1000 ) ) ) // relative time (fix bug #95999 on Boost 1.50)
  {
