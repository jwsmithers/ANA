  ...
  m_sqlStatement( sqlStatement )
{
  ...
  std::cout << "Create Statement " << this << ": " << m_sqlStatement << std::endl; // debug bug #98736
#endif
...
{
#ifdef ORACLE_CONNECTION_PROPERTIES_DEBUG
  std::cout << "Delete Statement " << this << std::endl; // debug bug #98736
#endif
  this->reset();
}
                                               ...
                                               sb4* errorCodePtr )
{
  if ( coral::MessageStream::msgVerbosity() == coral::Verbose )
  {
    ...
    if ( bindData.size() > 0 )
    {
      ...
      log << coral::Verbose << "Execute statement with no bind data" << coral::MessageStream::endmsg;
  }
  ...
  for ( coral::AttributeList::const_iterator iAttribute = bindData.begin(); iAttribute != bindData.end(); ++iAttribute, ++iVarInd )
  {
    ...
    *indicatorPointer = 0;
    ...
    {
      if ( m_sessionProperties->majorServerVersion() > 9 ) dty = SQLT_BFLOAT;
      else dty = SQLT_FLT;
    }
    else if ( attributeType == typeid(double) )
    {
      if ( m_sessionProperties->majorServerVersion() > 9 ) dty = SQLT_BDOUBLE;
      else dty = SQLT_FLT;
    }
    ...
    else // No supported variable type
    {
      ...
      delete [] indicators; // Fix Coverity RESOURCE_LEAK (bug #95667)
      return false;
    }
    // Bind the variable
...
bool
coral::OracleAccess::OracleStatement::fetchNext( bool failOutsideTx )
{
  ...
  if( failOutsideTx && !m_sessionProperties->isTransactionActive() )
