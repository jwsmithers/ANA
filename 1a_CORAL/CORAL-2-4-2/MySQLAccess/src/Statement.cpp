// $Id: Statement.cpp,v 1.47 2012-06-26 14:27:07 avalassi Exp $
...
namespace coral
{
  namespace MySQLAccess
  {
    ...
    Statement::Statement( boost::shared_ptr<const SessionProperties> sessionProperties, const std::string& sql )
    ...
    void Statement::execute( const coral::AttributeList& bindData )
    {
      coral::MessageStream log( m_sessionProperties->domainServiceName() );
      ...
      unsigned int num_rows;
      {
        ...
        if( mysql_query( m_sessionProperties->connectionHandle(), m_sqlStatement.c_str() ) )
        {
          ...
          if( errorHandler.lastErrorCode()  == ER_DUP_ENTRY )
            ...
            throw coral::SessionException( errorHandler.message(), "MySQLAccess::Statement", m_sessionProperties->domainServiceName() );
        }
    ...
    bool Statement::fetchNext()
    {
      ...
      if( !m_sessionProperties->isTransactionActive() )
        throw coral::QueryException( m_sessionProperties->domainServiceName(), "Transaction is not active", "fetchNext()" );
      ...
      for( unsigned int i = 0; i < num_fields; i++)
      {
        ...
        *///
          ...
          //f.type == MAX_NO_FIELD_TYPES )
        {
          ...
          throw UnSupportedSqlTypeException(m_sessionProperties->domainServiceName() , "MySQLAccess::Statement::getResultSetInfo", str.str());
        }
        // UNKNOWN
        else
        {
