...
#include "CoralBase/../src/coral_bind_headers.h"
#endif
#include "CoralBase/../src/coral_filesystem_headers.h"
...
#else
  std::function<void(const std::string&)> cb(std::bind(&coral::XMLLookupService::XMLLookupService::setInputFileName, this, std::placeholders::_1));
#endif
#ifdef CORAL240PM
...
{
  coral::lock_guard lock(m_mutexLock);
...
coral::XMLLookupService::XMLLookupService::initialize()
{
  ...
  try
  {
    //std::cout << "__XMLPlatformUtils::Initialize()" << std::endl; // debug bug #100599
  ...
  //std::cout << "__XMLPlatformUtils::Terminate()" << std::endl; // debug bug #100599
