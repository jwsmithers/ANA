...
public:
  ...
  setUp()
  {
    m_coralConnStringAdmin = CoralCppUnitDBTest::BuildUrl( "SQLite" );
    // This test must start with a clean file (bug #103502)
    boost::filesystem::remove( m_coralConnStringAdmin.substr(12) ); // strip 'sqlite_file:'
  }
  ...
  test_All()
  {
    ...
    {
      std::auto_ptr<coral::ISessionProxy> session2( connSvc.connect( m_coralConnStringAdmin ) );
