...
namespace coral
{
  namespace MySQLAccess
  {
    ...
    {
      SessionConfigurator( boost::shared_ptr<const SessionProperties> props )
        : m_properties( props )
      {
      }
      ...
      bool autocommit( bool OnOff )
      {
        ...
        if( mysql_autocommit( m_properties->connectionHandle(), (OnOff ? 1 : 0) ) )
          return false;
      ...
      bool txIsolationLevel( TxIsolationLevel level=ReadCommitted )
      {
        ...
        if( mysql_query( m_properties->connectionHandle(), sql.c_str() ) )
          return false;
    ...
    struct SchemaLoader
    {
      SchemaLoader( boost::shared_ptr<const SessionProperties> props )
        : m_properties( props )
      {
      }
      ...
      bool schemaExists( const std::string& schemaName )
      {
        ...
        if( (result = mysql_list_dbs( m_properties->connectionHandle(), NULL /* LIST ALL DBS */ )) == 0 )
        {
          coral::MySQLAccess::ErrorHandler errorHandler;
          MYSQL_HANDLE_ERR(errorHandler,m_properties->connectionHandle(),"Getting list of schemas");
          throw coral::SessionException( errorHandler.message(), "MySQLAccess::SchemaLoader", m_properties->domainServiceName() );
        }
...
coral::MySQLAccess::Session::Session( coral::IDevConnection& connection,
  ...
  , m_lock( conlock )
{
  m_properties.reset( new SessionProperties( domainProperties, connectionString, schemaName, handle, connected, conlock, *this, mode ) );
...
coral::MySQLAccess::Session::~Session()
{
  ...
  m_properties->nullifySession(); // fix bug #73834 aka big #80022!
  m_properties.reset();

  if( this->m_transaction != 0 ) delete this->m_transaction;

  if( this->m_monitorController != 0 ) delete this->m_monitorController;

  if( this->m_schema != 0 ) delete this->m_schema;
}

#ifndef CORAL300MG
coral::IMonitoring&
coral::MySQLAccess::Session::monitoring()
{
  return *m_monitorController;
}
#else
coral::IMonitoringController&
coral::MySQLAccess::Session::monitoringController()
{
  return *m_monitorController;
}
#endif
    ...
    if( ! m_properties->isConnected() )
    {
                                 ...
                                 CLIENT_FOUND_ROWS ) ) // This connect time setting ensures compatible behavior with OracleAccess for UPDATE
      {
        ...
        if( errorHandler.lastErrorCode() == CR_SERVER_LOST ||
          ...
          throw coral::StartSessionException( m_properties->domainServiceName(), errorHandler.message() );
        else
          throw coral::SessionException( m_properties->domainServiceName(), errorHandler.message(), "coral::MySQLAccess::startUserSession|connect" );
      }
    ...
    else
    {
      ...
      if ( mysql_change_user( handle, userName.c_str(), password.c_str(), 0 ) != 0 )
      {
        ...
        if( errorHandler.lastErrorCode() == CR_SERVER_LOST )
          ...
          throw coral::SessionException( m_properties->domainServiceName(), errorHandler.message(), "coral::MySQLAccess::startUserSession|re-authenticate" );
      }
