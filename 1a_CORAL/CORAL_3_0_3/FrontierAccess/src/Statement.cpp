...
namespace
{
  ...
  public:
      ...
      : m_time( 0 )
    {
      ...
      // OLD! Parse from 2006-1-12.15.47.0.0
      // NEW! Parse from 2006-01-12 15:47:00[.123456] (fix bug #98345)
      ...
      *pos02 = 0;
      ...
      // Get the day
      //pos02 = strchr( pos01, '.' );
      pos02 = strchr( pos01, ' ' ); // fix bug #98345
      if( !pos02 )
      ...
      *pos02 = 0;
      ...
      if( !pos02 )
      {
        //throw coral::Exception( "TimestampParser", "error in parsing the second from timestamp [" + std::string(timestamp) + "]", "coral::TimestampParser::constructor" ); // workaround for bug #98962
        second = atoi(pos01);
      }
      else
      {
  ...
  private:
    ...
    TimestampParser( const TimestampParser& rhs );

    /// Assignment operator is private (fix Coverity MISSING_ASSIGN)
    TimestampParser& operator=( const TimestampParser& rhs );
...
namespace coral
{
  ...
  {
    Statement::Statement( boost::shared_ptr<const SessionProperties> properties,
      ...
      , m_nulls()
      , m_metaData()
      , m_majorServerVersion( properties->majorServerVersion() )
      , m_preparedSqlStatement( sqlStatement )
      , m_sqlStatement( sqlStatement )
      , m_field( new frontier::AnyData )
      , m_mslog( 0 )
    {
      this->m_frontierSession = new frontier::Session( &(m_sessionProperties->frontierConnection()) );
    ...
    bool Statement::prepare( const std::string& sqlStatement )
    {
      ...
      m_preparedSqlStatement = sqlStatement;
      m_sqlStatement         = sqlStatement;
      mslog() << coral::Debug << "Prepared statement : \"" << m_sqlStatement << "\"" << coral::MessageStream::endmsg;
      return true;
    }
    ...
    bool Statement::bind( const coral::AttributeList& inputData )
    {
      ...
      for ( coral::AttributeList::const_iterator iColumn = (*m_boundInputData).begin(); iColumn != (*m_boundInputData).end(); ++iColumn )
      {
        ...
        //std::cout << "Type=" << colType.name() << std::endl;
        if( colType == typeid(std::string) )
        {
          ...
          value = "'" + value + "'";
          //std::cout << "Value=" << value << std::endl;
          //std::cout << "SQL=" << m_sqlStatement << std::endl;
        ...
        else if( colType == typeid(coral::Blob) )
        {
          ...
          // For now, throw instead of returning wrong results (CORALCOOL-2737)
          throw coral::Exception( m_sessionProperties->domainServiceName(), "BLOB bind variables are not supported", "coral::FrontierAccess::Statement::bind" );
        }
    ...
    bool Statement::execute( const coral::AttributeList& rowBuffer,
#ifndef CORAL300WC
                             bool forceReload
#else
                             int timeToLive
                             ...
                             )
    {
      if ( rowBuffer.size() > 0 )
      {
        ...
        rowBuffer.toOutputStream( msg );
        mslog() << coral::Debug << "Execute statement with bind data: " << msg.str() << coral::MessageStream::endmsg;
      }
      else
        mslog() << coral::Debug << "Execute statement with no bind data" << coral::MessageStream::endmsg;
      try
      {
        // Bind first
        ...
        if ( rowBuffer.size() > 0 )
          mslog() << coral::Debug << "Execute statement after binding data: " << m_sqlStatement << coral::MessageStream::endmsg;
        ...
        try
        {
          ...
          mslog() << coral::Verbose << "Running with the compression level: " << cachectrl.compressionLevel() << coral::MessageStream::endmsg;
        }
        catch( const std::exception& )
        {
          mslog() << coral::Verbose << "Running with the default compression level" << coral::MessageStream::endmsg;
        }

        {
          ...
          try
          {
            ...
            if( forceReload )
            {
              mslog() << coral::Verbose << "Refreshing data with query \"" << this->m_sqlStatement << "\"" << coral::MessageStream::endmsg;
              m_sessionProperties->frontierConnection().setReload(1);
            }
            else
            {
              ...
              m_sessionProperties->frontierConnection().setReload(0);
            }
#else
            // Set timeToLive value on the connection
            m_sessionProperties->frontierConnection().setTimeToLive( timeToLive );
            const char *ttlName = "";
            ...
            m_frontierSession->getData( m_listOfRequests );
            m_frontierSession->setCurrentLoad( 1 );
            ...
            while( ! (m_frontierSession->isEOR()) )
            {
              Field f;
              m_frontierSession->assignString( &(f.name) );
              m_frontierSession->assignString( &(f.type) );
              m_metaData.push_back( f );
              m_nulls.push_back( 0 );
              //           mslog() << coral::Verbose << f.name <<" => "<< f.type << coral::MessageStream::endmsg;
            }

            mslog() << coral::Verbose << "Result for query " << m_sqlStatement << " contains " << this->numberOfRowsProcessed() << " rows" << coral::MessageStream::endmsg;
            m_sessionProperties->log().push_back( coral::FrontierAccess::LogEntry( m_sqlStatement ) );
      ...
      {
        throw coral::QueryException( m_sessionProperties->domainServiceName(), e.what(), "coral::FrontierAccess::Statement::execute" );
      }
    ...
    {
      return( m_frontierSession->getRecNum() );
    }

    const std::type_info* Statement::typeForOutputColumn( int columnId ) const
    {
      const coral::ITypeConverter& typeConverter = m_sessionProperties->typeConverter();
    ...
    {
      m_sessionProperties->frontierConnection(); // hack to fix bug #79883 aka bug #80024
      int dataAvailable = 0;
      try
      {
        dataAvailable = m_frontierSession->next();
      }
      catch( const std::exception& e )
      {
        throw coral::QueryException( m_sessionProperties->domainServiceName(), e.what(), "coral::FrontierAccess::Statement::fetchNext" );
      }
      if( dataAvailable )
      {
        ++m_currentRow;
    ...
    void Statement::reset()
    {
      ...
      if( m_frontierSession != 0 )
        delete m_frontierSession;
    ...
    bool Statement::copyData()
    {
      ...
      for( size_t fieldIdx = 0; fieldIdx < numOfColumns; fieldIdx++ )
      {
        ...
        // Fill the field
        m_frontierSession->getAnyData( &field );
            ...
            if( field.getRawStrP() != NULL )
            {
              ...
              if( fieldType.find( "NUMBER" ) != std::string::npos )
              {
                ...
                else
                {
                  ...
                  throw coral::QueryException( m_sessionProperties->domainServiceName(), msg.str(), "coral::FrontierAccess::Statement::copyData" );
                }
              }
              ...
              else
              {
                ...
                throw coral::QueryException( m_sessionProperties->domainServiceName(), msg, "coral::FrontierAccess::Statement::copyData" );
              }
            }
            else
            {
    ...
    {
      if( this->m_mslog == 0 )
        this->m_mslog = new  coral::MessageStream( m_sessionProperties->domainServiceName() );
