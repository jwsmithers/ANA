...
{
  for ( std::vector< coral::AttributeSpecification* >::const_iterator iSpecification = rhs.m_attributeSpecifications.begin(); iSpecification != rhs.m_attributeSpecifications.end(); ++iSpecification )
  {
...
coral::AttributeListSpecification::~AttributeListSpecification()
{
  for ( std::vector< coral::AttributeSpecification* >::iterator iSpecification = m_attributeSpecifications.begin(); iSpecification != m_attributeSpecifications.end(); ++iSpecification )
    delete *iSpecification;
}
...
#ifdef CORAL240AL
bool
coral::AttributeListSpecification::exists( const std::string& name ) const
{
  std::map<std::string, int>::const_iterator iSpecificationForAttribute = m_mapOfNameToIndex.find( name );
  if ( iSpecificationForAttribute == m_mapOfNameToIndex.end() ) return false;
  else return true;
}
#endif
