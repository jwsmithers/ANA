    ...
    {
      {
        ...
        TcpSocketPtr tmp = TcpSocket::listenOn( "localhost", getTestPort() );
        // creating a second listening socket on this port should fail
        CPPUNIT_ASSERT_THROW( TcpSocket::listenOn( "localhost", getTestPort() ),
...

      {
        ...
        // a new one
        TcpSocketPtr listen1 = TcpSocket::listenOn( "localhost", getTestPort() );
...

      {
...

        {
          ...
          for (int i = 0; i<nConnections; i++)
            connectTo[i] = TcpSocket::connectTo( "localhost", getTestPort() );
    ...
    {
      {
        ...
        CPPUNIT_ASSERT_THROW( TcpSocket::connectTo( "localhost", getTestPort() ),
                              GenericSocketException );

        // test host resolving
        CPPUNIT_ASSERT_THROW( TcpSocket::connectTo( "invalid host name", getTestPort()),
                              GenericSocketException );
        CPPUNIT_ASSERT_THROW( TcpSocket::connectTo( "127.0.0.1:300", getTestPort()),
                              GenericSocketException );
    ...
    {
      {
        ...
        {
          int res=0;
          //const int len=30000; // Local variable "message" uses 30000 bytes of stack space, which exceeds the maximum single use of 10000 bytes
          const int len=10000; // Fix Coverity STACK_USE
