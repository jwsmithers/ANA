...
namespace coral
{
  ...
  namespace CoralStubs {
    ...
    public:
      ...
      IRowIteratorPtr fetchRows( Token sessionID, const QueryDefinition& qd, AttributeList* pRowBuffer, unsigned int rowCacheSize, unsigned int memoryCacheSize ) const;

      IRowIteratorPtr fetchRows( Token sessionID, const QueryDefinition& qd, const std::map< std::string, std::string > outputTypes, unsigned int rowCacheSize, unsigned int memoryCacheSize ) const;
