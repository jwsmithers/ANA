...
namespace coral
{
  ...
  public:
    ...
    test_SISimpleTypesBuffer(size_t buffersize)
    {
      ...
      coral::CoralStubs::DummyByteBufferIterator* dbi = new coral::CoralStubs::DummyByteBufferIterator;
      ...
      swi.flush();
      while(swi.nextBuffer()) dbi->addBuffer(swi.currentBuffer());
...
#if __CORALSTUBS_SIZEOF_LONG__ == 4
      swi.append((uint64_t) 0xFFFFFFFF);
#elif __CORALSTUBS_SIZEOF_LONG__ == 8
...
#error unknown long max
    ...
    {
      coral::CoralStubs::SegmentWriterIterator* swi = new coral::CoralStubs::SegmentWriterIterator(coral::CALOpcodes::ConnectRO, true, false, buffersize);
      ...
      attr01[18].data<coral::TimeStamp>() = coral::TimeStamp(1972, 1, 1, 12, 32, 34, 0);
      ...
      for ( size_t j = 0; j < maxc; ++j, ++p )
        *p = rn256(rng);  // Fix Coverity SECURE_CODING
