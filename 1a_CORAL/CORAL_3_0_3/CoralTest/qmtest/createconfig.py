...
class BasicTest:
    ...
    def writeQmt(self, backendPair):
        ...
        if backendPair:
            ...
            backend_args = ''
            backend_suffix = ''
        # Build the possible paths to the reference output
        refpath1 = testname[1]
        for h in range(2, len(testname)): refpath1 += '_' + testname[h]
        refpath1 = ('../../Tests/%s/' % refpath1)
        refpath2 = 'referenceLogs/'
        # Build the possible names of the reference output
        reffile1 = self.name + backend_suffix + '.ref'
        reffile2 = self.name + '.ref'
        # Check if we have a reference output
        ref = False
        if os.path.exists( refpath1 + reffile1 ):
            ref = True
            refpath = refpath1
            reffile = reffile1
        elif os.path.exists( refpath2 + reffile1 ):
            ref = True
            refpath = refpath2
            reffile = reffile1
        elif os.path.exists( refpath1 + reffile2 ):
            ref = True
            refpath = refpath1
            reffile = reffile2
        elif os.path.exists( refpath2 + reffile2 ):
            ref = True
            refpath = refpath2
            reffile = reffile2
