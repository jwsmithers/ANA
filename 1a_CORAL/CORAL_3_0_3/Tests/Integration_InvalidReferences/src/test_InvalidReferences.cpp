  ...
  public:
    ...
    void test_bug73834()
    {
      ...
      try
      {
        ...
        std::string tableName = testTableName();
        {
          ...
          std::cout << "*** Curr row: " << cursor.currentRow() << std::endl;
    ...
    void test_bug79983() // formerly test_bug73834b
    {
      ...
      try
      {
        ...
        //std::cout << "*** Now purge connection pool" << std::endl; // crash?
        //connSvc.purgeConnectionPool(); // no need, conn deleted immediately
        std::cout << "*** Now increment cursor" << std::endl;
        try
        {
          CPPUNIT_ASSERT_EQUAL_MESSAGE( "Row1 exists", true, cursor.next() );
          CPPUNIT_ASSERT_EQUAL_MESSAGE( "Row2 exists", true, cursor.next() );
    ...
    ~CoralInvalidReferencesTest()
    {
      ...
      if ( m_itest==s_ntests )
      {
        coral::sleepSeconds(1); // Improve printout: allow last SessionDeleter to complete
        std::cout << "====== FINAL CLEANUP: delete the LAST test ======" << std::endl;
        // --- DROP THE RELEVANT TEST TABLES ---
        std::cout << "*** Drop the test tables: now start a RW session" << std::endl;
        //std::cout << "*** Connect RW" << std::endl;
        coral::ConnectionService connSvc;
        std::auto_ptr<coral::ISessionProxy>
          sessionRW( connSvc.connect( UrlRW(), coral::Update ) );
        ...
        //std::cout << "*** Disconnect RW" << std::endl;
        sessionRW->transaction().commit();
        std::cout << "*** Dropped the test tables: now close the RW session" << std::endl;
        sessionRW.reset();
        std::cout << "*** Closed the RW session: final cleanup completed" << std::endl;
      }
    }
