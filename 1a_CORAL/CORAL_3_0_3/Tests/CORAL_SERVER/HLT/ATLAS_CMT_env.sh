...
if [ "$1" == "" ] || [ "$2" != "" ]; then
  ...
  echo "Usage: . ATLAS_CMT_env.sh X.Y.Z[.W]"
  return 1
else
  export REL=$1
  shift
...
if [[ $majorREL < 19 ]]; then
  ...
  . $AtlasSetup/scripts/asetup.sh AtlasP1HLT,$REL,$dbg >$log 2>&1
else
  . $AtlasSetup/scripts/asetup.sh AtlasP1HLT,$REL,gcc48,$dbg >$log 2>&1
fi
