...
TestEnv::setupEnv()
{
  ...
  if ( !tns_admin )
    ::putenv( (char*)"TNS_ADMIN=/afs/cern.ch/sw/lcg/app/releases/CORAL/internal/oracle/admin" );
#else
...
TestEnv::dumpEnv()
{
  ...
  const char* path_lookup = ::getenv( "CORAL_DBLOOKUP_PATH" );
  if (!path_lookup) throw coral::Exception( "CORAL_DBLOOKUP_PATH is not set!", "TestEnv", "TestEnv::dumpEnv" );  // Fix Coverity NULL_RETURNS
  const char* path_auth = ::getenv( "CORAL_AUTH_PATH" );
  if (!path_auth) throw coral::Exception( "CORAL_AUTH_PATH is not set!", "TestEnv", "TestEnv::dumpEnv" );  // Fix Coverity NULL_RETURNS
  const char* tns_admin = ::getenv( "TNS_ADMIN" );
  if (!tns_admin) throw coral::Exception( "TNS_ADMIN is not set!", "TestEnv", "TestEnv::dumpEnv" );  // Fix Coverity NULL_RETURNS
