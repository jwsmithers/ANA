...
Testing::~Testing()
{
  if ( m_rowBuffer ) delete m_rowBuffer;  // fix bug #100601
  deleteSession();
}
...
Testing::fillData( int i )
{
  ...
  //row[17].data<coral::TimeStamp>() = coral::TimeStamp( 2006, 1, 12, 15, 47, i%60, 123456000 ); // more complex test for bug #98345 (see bug #98962)
...
Testing::checkData( int i ) const
{
  ...
  coral::TimeStamp expTS( 2006, 1, 12, 15, 47, i%60, 0 );
  //coral::TimeStamp expTS( 2006, 1, 12, 15, 47, i%60, 123456000 ); // more complex test for bug #98345 (see bug #98962)
  std::cout << "Expected TS  " << expTS.toString() << std::endl;
  std::cout << "Retrieved TS " << row[17].data<coral::TimeStamp>().toString() << std::endl;
  if ( row[17].data<coral::TimeStamp>() != expTS )
  {
