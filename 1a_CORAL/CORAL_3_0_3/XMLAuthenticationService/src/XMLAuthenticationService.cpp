...
coral::XMLAuthenticationService::XMLAuthenticationService::XMLAuthenticationService( const std::string& componentName )
  : coral::Service( componentName ),
    ...
    m_data()
{
  //std::cout << "Create XMLAuthenticationService" << std::endl; // debug bug #98958
#ifndef CORAL300CPP11
...
#else
  std::function<void(const std::string&)> cb(std::bind(&coral::XMLAuthenticationService::XMLAuthenticationService::setInputFileName, this, std::placeholders::_1));
#endif
#ifdef CORAL240PM
...
{
  //std::cout << "Delete XMLAuthenticationService" << std::endl; // debug bug #98958
#ifdef CORAL240PM
  coral::Property* pm = dynamic_cast<coral::Property*>(coral::Context::instance().propertyManager().property("AuthenticationFile"));
#else
  coral::Property* pm = dynamic_cast<coral::Property*>(coral::Context::instance().PropertyManager().property("AuthenticationFile"));
  ...
  for ( std::map< std::string, coral::XMLAuthenticationService::DataSourceEntry* >::iterator iConnection = m_data.begin();
        iConnection != m_data.end(); ++iConnection )
