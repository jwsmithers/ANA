...
// [This is concatenated to this file to appear verbatim in .so (bug #103539)]
...
/*
// Aborted attempt to work around c++ exception catching in python with root6
// See http://en.wikipedia.org/wiki/Setjmp.h
// See also http://stackoverflow.com/questions/9225415
// Alternatively see http://stackoverflow.com/questions/564060 and
// http://www.dabeaz.com/coroutines/index.html (Python Essential Reference)

// EXAMPLE:
// from PyCool import cool
// spec=cool.RecordSpecification()
// spec.extend("UI",cool.StorageType.UInt32)
// rec=cool.Record(spec)
// if cool.PyCool.Helpers.PyCoolTry.pyCoolTry():
//     rec.field(0).setValue(-1) # aborts (throw c++ exception)
// else:
...
namespace cool
{
  namespace PyCool
  {
    namespace Helpers
    {
      ...
      static jmp_buf PyCoolTryBuffer;

      class PyCoolTry
      {
      public:
        static bool pyCoolTry()
        {
          static bool first = true;
          if ( first ) 
          {
            signal( SIGABRT, pyCoolCatch );
            first = false;
          }
          return ( ! setjmp( cool::PyCool::Helpers::PyCoolTryBuffer ) );
        }
      private:
        PyCoolTry();
        static jmp_buf PyCoolTryBuffer;
        static void pyCoolCatch( int sig )
        {
          std::cout << "PyCoolCatch: handling " << sig << std::endl;
          // Jump back to where setjmp was called - making setjmp now return 1
          // See http://en.wikipedia.org/wiki/Setjmp.h
          longjmp( cool::PyCool::Helpers::PyCoolTryBuffer, 1 );
        }
      };
