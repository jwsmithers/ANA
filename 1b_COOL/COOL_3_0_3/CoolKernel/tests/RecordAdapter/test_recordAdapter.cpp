...
public:
  ...
  template<typename flt> void test_bug72147()
  {
    ...
    {
      name = "A_Double";
      ptype = &( StorageType::storageType( cool_StorageType_TypeId::Double ) );
    }
    else if ( typeid(flt) == typeid(float) )
    {
      name = "A_Float";
      ptype = &( StorageType::storageType( cool_StorageType_TypeId::Float ) );
    }
    else
    {
  ...
  void test_bug36646()
  {
    ...
    try
    {
      ...
      RecordSpecification spec;
      spec.extend( namePub, cool_StorageType_TypeId::String255 );
