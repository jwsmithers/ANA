...
namespace cool
{
  ...
  public:
    ...
    folderSchemaVersion( PayloadMode::Mode payloadMode )
    {
      ...
      // - COOL_2_9/COOL_3_0 may create '2.0.1', '2.8.0' and '2.9.0' folders,
      //   the latter being folders with vector payload in a separate table.
      if ( payloadMode == cool_PayloadMode_Mode::INLINEPAYLOAD ) return "2.0.1";
      else if ( payloadMode == cool_PayloadMode_Mode::SEPARATEPAYLOAD ) return "2.8.0";
      else if ( payloadMode == cool_PayloadMode_Mode::VECTORPAYLOAD ) return "2.9.0";
      else
      {
        std::stringstream s;
        s << "Invalid payload mode specified: " << payloadMode;
        throw InvalidFolderSpecification( s.str(), "RelationalFolder::folderSchemaVersion" );
      }
    }
    ...
    {
      if ( payloadMode == cool_PayloadMode_Mode::INLINEPAYLOAD ) return 0;  // inline
    ...
    RelationalFolder( const RelationalDatabasePtr& db,
      ...
      , m_folderSpec( versioningMode( row ), payloadSpecification( row ),
                      ...
                      payloadMode( row )
#else
                      payloadMode( row ) == cool_PayloadMode_Mode::SEPARATEPAYLOAD
#endif
                      )
  ...
  public:
    ...
    *///
    ...
    /// Store an object in a given channel with a given IOV and payload.
