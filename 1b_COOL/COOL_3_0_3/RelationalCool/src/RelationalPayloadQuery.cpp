                                           ...
                                           Record& whereData ) const
{
  ...
  if ( ref.isNull() )
  {
    ...
    if ( ref.specification().storageType().id() == cool_StorageType_TypeId::String255 ||
         ref.specification().storageType().id() == cool_StorageType_TypeId::String4k  ||
         ref.specification().storageType().id() == cool_StorageType_TypeId::String64k ||
         ref.specification().storageType().id() == cool_StorageType_TypeId::String16M )
      throw InternalErrorException
  ...
  else
  {
    ...
    if ( ref.specification().storageType().id() == cool_StorageType_TypeId::String255 ||
         ...
         ref.specification().storageType().id() == cool_StorageType_TypeId::String16M )
    {
      if ( rel != cool_FieldSelection_Relation::EQ && rel != cool_FieldSelection_Relation::NE )
        throw InternalErrorException
      ...
      if ( ref.data<std::string>() == "" )
      {
        ...
        if ( m_technology == "Oracle" || m_technology == "Frontier" )
        {
          ...
          whereClause += ref.specification().name();
          whereClause += " ";
          if ( rel == cool_FieldSelection_Relation::EQ )
          {
            whereClause +=
              FieldSelection::describe( cool_FieldSelection_Nullness::IS_NULL );
          }
          else
          {
            whereClause +=
              FieldSelection::describe( cool_FieldSelection_Nullness::IS_NOT_NULL );
          }
        }
