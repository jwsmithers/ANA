...
bool RelationalFolder::isSupportedSchemaVersion( const VersionNumber& rhs )
{
  ...
  VersionNumber lhs201 = folderSchemaVersion( cool_PayloadMode_Mode::INLINEPAYLOAD );
  if ( ( lhs201.majorVersion() == rhs.majorVersion() )
       && ( lhs201.minorVersion() == rhs.minorVersion() )
       //&& ( lhs201.patchVersion() <= rhs.patchVersion() )
       ) return true;
  VersionNumber lhs280 = folderSchemaVersion( cool_PayloadMode_Mode::SEPARATEPAYLOAD );
  if ( ( lhs280.majorVersion() == rhs.majorVersion() )
       && ( lhs280.minorVersion() == rhs.minorVersion() )
       //&& ( lhs280.patchVersion() <= rhs.patchVersion() )
       ) return true;
#ifdef COOL290VP
  VersionNumber lhs290 = folderSchemaVersion( cool_PayloadMode_Mode::VECTORPAYLOAD );
...
RelationalFolder::payloadMode( const coral::AttributeList& folderRow )
{
  ...
  VersionNumber lhsSeparate = folderSchemaVersion( cool_PayloadMode_Mode::SEPARATEPAYLOAD );
  if ( ( lhsSeparate.majorVersion() == schemaVersion.majorVersion() )
       && ( lhsSeparate.minorVersion() == schemaVersion.minorVersion() )
       //&& ( lhs280.patchVersion() <= rhs.patchVersion() )
       )
    return cool_PayloadMode_Mode::SEPARATEPAYLOAD;

  VersionNumber lhsVect = folderSchemaVersion( cool_PayloadMode_Mode::VECTORPAYLOAD );
  if ( ( lhsVect.majorVersion() == schemaVersion.majorVersion() )
       && ( lhsVect.minorVersion() == schemaVersion.minorVersion() )
       //&& ( lhs280.patchVersion() <= rhs.patchVersion() )
       )
    return cool_PayloadMode_Mode::VECTORPAYLOAD;

  return cool_PayloadMode_Mode::INLINEPAYLOAD;
}
...
void RelationalFolder::initialize( const coral::AttributeList& row )
{
  ...
  if ( m_folderSpec.versioningMode() == cool_FolderVersioning_Mode::MULTI_VERSION )
  {
...
( const std::string& tagName ) const
{
  ...
  if ( this->versioningMode() == cool_FolderVersioning_Mode::SINGLE_VERSION )
...
void RelationalFolder::deleteTag( const std::string& tagName )
{
  ...
  // SV folder
  if ( this->versioningMode() != cool_FolderVersioning_Mode::MULTI_VERSION ) {
  ...
  PayloadMode::Mode pMode ) const
{
          ...
          row = rows.begin(); row != rows.end(); ++row )
  {
    ...
    data[RelationalObject2TagTable::columnNames::iovUntil].setValue
      ( row->until() );
    if ( this->payloadMode() == cool_PayloadMode_Mode::SEPARATEPAYLOAD )
      data[RelationalObject2TagTable::columnNames::payloadId].setValue
        ( row->payloadId() );
    else if ( this->payloadMode() == cool_PayloadMode_Mode::VECTORPAYLOAD )
