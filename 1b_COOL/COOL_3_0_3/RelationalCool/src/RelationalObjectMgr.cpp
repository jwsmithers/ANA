  ...
  const bool countOnly ) const
{
  ...
  try
  {
    ...
    else if ( versioningMode == cool_FolderVersioning_Mode::MULTI_VERSION )
    {
  ...
  bool hasNewData ) const
{
  ...
  if ( hasNewData )
  {
    ...
    RecordSpecification whereDataSpec;
    whereDataSpec.extend( "hasNewData", cool_StorageType_TypeId::Bool );
    Record whereData( whereDataSpec );
...
{
  if ( folder->versioningMode() != cool_FolderVersioning_Mode::SINGLE_VERSION )
  ...
  const std::vector<RelationalPayloadTableRowPtr>& payloadRows ) const
{
  ...
  if ( folder->payloadMode() == cool_PayloadMode_Mode::VECTORPAYLOAD  )
  {
  ...
  else if ( folder->payloadMode() == cool_PayloadMode_Mode::SEPARATEPAYLOAD  )
  {
    ...
    // iov table
    RecordSpecification iovTableSpec =
      RelationalObjectTable::tableSpecification( rowSpec, cool_PayloadMode_Mode::SEPARATEPAYLOAD );
    Record iovData( iovTableSpec );
    // TEMPORARY HACK: no IField::setValue/fastCopy(const coral::Attribute&)...
    coral::AttributeList& iovDataAL =
      const_cast<coral::AttributeList&>( iovData.attributeList() );
  ...
  else
  {
    ...
    RecordSpecification tableSpec =
      RelationalObjectTable::tableSpecification( rowSpec, cool_PayloadMode_Mode::INLINEPAYLOAD );
  ...
  const std::vector<RelationalObjectPtr>& objects ) const
{
  ...
  if ( folder->payloadMode() == cool_PayloadMode_Mode::SEPARATEPAYLOAD )
  {
  ...
  bool userTagOnly ) const
{
      ...
      try
      {
        ...
        HvsTagLock::Status lockStatus =
          ...
          ( tagRow[RelationalGlobalTagTable::columnNames::tagLockStatus]
            .data<UInt16>() );
        if ( lockStatus == cool_HvsTagLock_Status::LOCKED )
          throw TagIsLocked
            ( "Cannot store objects with user tag '" + userTagName +
              "': tag is locked", "RelationalObjectMgr" );
        if ( lockStatus == cool_HvsTagLock_Status::PARTIALLYLOCKED )
  ...
  boost::shared_ptr<RelationalSequence> payloadIdSeq;
  RelationalPayloadTable::columnTypes::payloadId pIdOffset=0;
  if ( // Quick and dirty patch with equivalent functionality after removing RelationaFolder::hasPayloadTable (CORALCOOL-2736)
#ifdef COOL290VP
      folder->folderSpecification().payloadMode() == cool_PayloadMode_Mode::SEPARATEPAYLOAD
#else
      folder->folderSpecification().hasPayloadTable()
#endif
       )
  {
    payloadIdSeq = queryMgr().sequenceMgr().getSequence
