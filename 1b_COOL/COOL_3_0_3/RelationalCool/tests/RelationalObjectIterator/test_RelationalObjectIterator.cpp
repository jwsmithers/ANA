...
public:
  ...
  {
    fillFolders();
    ChannelSelection ch( 0, 0, cool_ChannelSelection_Order::sinceDescBeforeChannel );
  ...
  RelationalObjectIteratorTest() : CoolDBUnitTest(), payloadSpec()
  {
    ...
    payloadSpec.extend("CLOB",cool_StorageType_TypeId::String64k);
  }
...
private:
  ...
  void createFolders()
  {
    ...
    FolderSpecification fSv( cool_FolderVersioning_Mode::SINGLE_VERSION, payloadSpec );
    s_db->createFolder( "/fSV", fSv, "SV folder" );
#ifdef COOL290VP
    // Create the MV folder with vector payload
    FolderSpecification fmMv( cool_FolderVersioning_Mode::MULTI_VERSION, payloadSpec, cool_PayloadMode_Mode::VECTORPAYLOAD );
    s_db->createFolder( "/fMVv", fmMv, "MV folder with vector payload" );
    // Create the SV folder with vector payload
    FolderSpecification fmSv( cool_FolderVersioning_Mode::SINGLE_VERSION, payloadSpec, cool_PayloadMode_Mode::VECTORPAYLOAD );
