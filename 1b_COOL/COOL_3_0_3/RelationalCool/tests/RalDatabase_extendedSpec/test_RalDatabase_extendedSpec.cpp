...
public:
  ...
  void test_storagObject_special_characters()
  {
    ...
    spec.extend( "quotes", cool_StorageType_TypeId::String255 );
    spec.extend( "umlauts", cool_StorageType_TypeId::String255 );
    spec.extend( "diacriticals", cool_StorageType_TypeId::String255 );
    Record payload( spec );
    IFolderPtr folder = s_db->createFolder( "/myfolder", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) );
  ...
  void test_supportedTypes()
  {
    try
    {
      ...
      for ( unsigned int i=0; i<spec.size(); i++ )
      {
        ...
        if ( fldSpec.storageType() == cool_StorageType_TypeId::Float )
        {
        ...
        // TEMPORARY! You should be able to use higher precision!
        else if ( fldSpec.storageType() == cool_StorageType_TypeId::Double )
        {
        ...
        else if ( fldSpec.storageType() == cool_StorageType_TypeId::Blob64k )
        {
        ...
        else if ( fldSpec.storageType() == cool_StorageType_TypeId::Blob16M )
        {
          // use native comparison for blobs
          val1_in = "true";
  ...
  void test_default_UInt63()
  {
    ...
    StorageType::TypeId attrType = cool_StorageType_TypeId::UInt63;
  ...
  void test_extSpec_string255()
  {
    ...
    StorageType::TypeId attrType = cool_StorageType_TypeId::String255;
    spec.extend( attrName, attrType );
  ...
  void test_extSpec_allStrings()
  {
    ...
    std::vector<pair_type> all_types;
    all_types.push_back
      (pair_type ("A_STRING255", cool_StorageType_TypeId::String255 ) );
    all_types.push_back
      (pair_type ("A_STRING4K",  cool_StorageType_TypeId::String4k ) );
    all_types.push_back
      (pair_type ("A_STRING64K", cool_StorageType_TypeId::String64k ) );
    all_types.push_back
      (pair_type ("A_STRING16M", cool_StorageType_TypeId::String16M ) );
    RecordSpecification spec;
  ...
  void test_default_string4000()
  {
    RecordSpecification spec;
    std::string attrName = "S";
    StorageType::TypeId attrType = cool_StorageType_TypeId::String4k;
    spec.extend( attrName, attrType );
    IFolderPtr folder = s_db->createFolder( "/myfolder", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) );
  ...
  void test_payloadSpecDescription()
  {
    ...
    for ( int i = 0; i < 900; ++i )
    {
      ...
      spec.extend( s.str(), cool_StorageType_TypeId::String64k );
    }
    CPPUNIT_ASSERT_EQUAL_MESSAGE
  ...
  void test_payloadSpecTooManyFields()
  {
    ...
    for ( int i = 0; i < 900; ++i )
    {
      std::stringstream s;
      s << "S" << i;
      spec.extend( s.str(), cool_StorageType_TypeId::String4k );
    }
    s_db->createFolder( "/myfolder900", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) );
    {
      int i = 900;
      std::stringstream s;
      s << "S" << i;
    ...
    CPPUNIT_ASSERT_THROW( s_db->createFolder( "/myfolder901", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) ),
  ...
  void test_payloadSpecTooManyBlobFields()
  {
    ...
    for ( int i = 0; i < 10; ++i )
    {
      std::stringstream s;
      s << "B" << i;
      spec.extend( s.str(), cool_StorageType_TypeId::Blob64k );
    }
    s_db->createFolder( "/myfolder10blob", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) );
    {
      int i = 11;
      std::stringstream s;
      s << "B" << i;
      //spec.extend( s.str(), cool_StorageType_TypeId::Blob64k );
      spec.extend( s.str(), cool_StorageType_TypeId::Blob16M );
    }
    CPPUNIT_ASSERT_THROW( s_db->createFolder( "/myfolder11blob", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) ),
  ...
  {
    {
      ...
      IFolderPtr folder = s_db->createFolder( "/myfolder10clob", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) );
      Record record( spec );
    ...
    else
    {
      ...
      IFolderPtr folder = s_db->createFolder( "/myfolder11clob", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) );
      Record record( spec );
      std::string value = std::string( 1000, 'x' );
      for ( int i = 0; i < 11; ++i )
      {
  ...
  void test_payloadSpecTooManyString255Fields()
  {
    ...
    for ( int i = 0; i < 200; ++i )
    {
      std::stringstream s;
      s << "S" << i;
      spec.extend( s.str(), cool_StorageType_TypeId::String255 );
    }
    ...
    for ( int i = 0; i < 699; ++i )
    {
      std::stringstream s;
      s << "I" << i;
      spec.extend( s.str(), cool_StorageType_TypeId::Int64 );
    }
    s_db->createFolder( "/myfolder200String255", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) );
    {
      int i = 200;
      std::stringstream s;
      s << "S" << i;
    ...
    CPPUNIT_ASSERT_THROW( s_db->createFolder( "/myfolder201String255", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) ),
  ...
  void test_payloadSpecInvalidFieldName()
  {
    ...
    {
      RecordSpecification spec;
      spec.extend( "S12345678901234567890123456789", cool_StorageType_TypeId::String255 );
      s_db->createFolder( "/myfolder", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) );
      spec.extend( "S123456789012345678901234567890", cool_StorageType_TypeId::String255 );
      CPPUNIT_ASSERT_THROW( s_db->createFolder( "/myfoldernew", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) ),
    ...
    // Test that a field name of size 0 fails
    {
      ...
      try
      {
        spec.extend( "", cool_StorageType_TypeId::String255 );
    ...
    {
      RecordSpecification spec;
      spec.extend( "is-a", cool_StorageType_TypeId::String255 );
      CPPUNIT_ASSERT_THROW( s_db->createFolder( "/myfoldernew", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) ),
                            PayloadSpecificationInvalidFieldName );
    }
    ...
    {
      RecordSpecification spec;
      spec.extend( "_payload", cool_StorageType_TypeId::String255 );
      CPPUNIT_ASSERT_THROW( s_db->createFolder( "/myfoldernew", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) ),
                            PayloadSpecificationInvalidFieldName );
    }
    ...
    {
      RecordSpecification spec;
      spec.extend( "1payload", cool_StorageType_TypeId::String255 );
      CPPUNIT_ASSERT_THROW( s_db->createFolder( "/myfoldernew", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) ),
                            PayloadSpecificationInvalidFieldName );
    }
    ...
    {
      RecordSpecification spec;
      spec.extend( "COOL_payload", cool_StorageType_TypeId::String255 );
      CPPUNIT_ASSERT_THROW( s_db->createFolder( "/myfoldernew", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) ),
                            PayloadSpecificationInvalidFieldName );
    }
    // Test that a field name starting with "cool_" fails
  ...
  void test_emptyString()
  {
    ...
    rspec.extend( name, cool_StorageType_TypeId::String255 );
    FolderSpecification fspec( rspec );
#else
    FolderSpecification fspec;
    fspec.payloadSpecification().extend
      ( name, cool_StorageType_TypeId::String255 );
#endif
  ...
  void test_blob64kOver64k()
  {
    RecordSpecification spec;
    std::string name = "B";
    StorageType::TypeId attrType = cool_StorageType_TypeId::Blob64k;
  ...
  void test_blob16MOver64k()
  {
    RecordSpecification spec;
    std::string name = "B";
    StorageType::TypeId attrType = cool_StorageType_TypeId::Blob16M;
    spec.extend( name, attrType );
    IFolderPtr folder = s_db->createFolder( "/myfolder", FolderSpecification( cool_FolderVersioning_Mode::SINGLE_VERSION, spec ) );
...
#ifdef COOL290VP
    RecordSpecification rspec;
    rspec.extend( name, cool_StorageType_TypeId::Blob64k );
    FolderSpecification fspec( rspec );
#else
    FolderSpecification fspec;
    fspec.payloadSpecification().extend
      ( name, cool_StorageType_TypeId::Blob64k );
#endif
    IFolderPtr folder = s_db->createFolder( "/myfolder", fspec );
    CPPUNIT_ASSERT( folder.get() != 0 );
    Record record( fspec.payloadSpecification() );
