                                  ...
                                  const std::string& lastUpdate )
{
  ...
  std::string orderClause =
    folder.payloadMode() == cool_PayloadMode_Mode::SEPARATEPAYLOAD  ?
...
std::string Replication::serverTime( const RalDatabase& db )
{
  ...
  queryDef.addSelectItem
    ( serverTimeClause, cool_StorageType_TypeId::String255, serverTimeAlias );
  const std::vector<RelationalTableRow> rows
                                   ...
                                   const std::string& lastUpdate )
{
  ...
  if ( sourceFolder.payloadMode() == cool_PayloadMode_Mode::SEPARATEPAYLOAD )
  {
    boost::shared_ptr<CursorHandle>
  ...
  if ( sourceFolder.payloadMode() == cool_PayloadMode_Mode::VECTORPAYLOAD )
  {
    boost::shared_ptr<CursorHandle>
      cursorHandle = newPayloadTableRows( sourceDb(),
                                          sourceFolder,
  ...
  if ( sourceFolder.versioningMode() == cool_FolderVersioning_Mode::MULTI_VERSION )
  {
