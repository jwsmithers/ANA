      ...
      try {
        ...
        tables.push_back
          ( RelationalObjectTable::sequenceName
            ( relFolder->objectTableName() ) );
        // Payload table and sequence (separate payload only)
        // Replace RelationalFolder::hasPayloadTable - see CORALCOOL-2736
        if (
#ifdef COOL290VP
        ...
        // Payload table and sequence (vector payload) - fix bug CORALCOOL-2736
#ifdef COOL290VP
        if ( folder->folderSpecification().payloadMode() == cool_PayloadMode_Mode::VECTORPAYLOAD )
        {
          // Table only, not sequence table
          tables.push_back
            ( relFolder->payloadTableName() );
        }
#endif
        ...
        if ( relFolder->versioningMode() == cool_FolderVersioning_Mode::MULTI_VERSION ) {
          tables.push_back
    ...
    for ( node = nodes.begin(); node != nodes.end(); node++ ) {
      try {
        ...
        if ( priv == INSERT ) {
          ...
          tables.push_back
            ( relFolder->channelTableName() );
          // INSERT: payload table (separate and vector payload only)
          if (
#ifdef COOL290VP
              folder->folderSpecification().payloadMode() == cool_PayloadMode_Mode::SEPARATEPAYLOAD
              || folder->folderSpecification().payloadMode() == cool_PayloadMode_Mode::VECTORPAYLOAD // bug fix: add also for vector payload (CORALCOOL-2736)
#else
              folder->folderSpecification().hasPayloadTable() // as before
#endif
              )
          {
            tables.push_back
          ...
          tables.push_back
            ( relFolder->channelTableName() );
          // UPDATE: payload table sequence (separate payload only)
          // Replace RelationalFolder::hasPayloadTable - see CORALCOOL-2736
          if (
#ifdef COOL290VP
              folder->folderSpecification().payloadMode() == cool_PayloadMode_Mode::SEPARATEPAYLOAD
