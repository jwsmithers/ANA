...
class StandardTest(ShellCommandTest):
    ...
    def Run(self, context, result):
        ...
        dependency_vars = { 'LOCALRT'    : 'COOL',
                            'CORAL_BASE' : 'CORAL',
                            'ROOT_BASE'  : 'ROOT',
                            'REFLEX_BASE': 'Reflex' }
        for vname in dependency_vars:
            if vname in os.environ:
                result['COOLTest.software_versions.%s'%dependency_vars[vname]] = os.environ[vname]
...
class DatabaseTest(StandardTest):
    ...
    def DBName(self, type = 0):
        ...
        if 'LOCALRT' in os.environ:
            id = os.environ['LOCALRT'] + self.GetPlatform()
        else:
...
class EvolutionTest(DatabaseTest):
    ...
    def PrepareTmpPackage(self,destdir,env):
        ...
        cool_current_dir = os.path.join(env["LOCALRT"],self.GetPlatform())
...
path_remove PATH '%(LOCALRT)s'
path_remove LD_LIBRARY_PATH '%(LOCALRT)s' WIN32 ''
path_remove PYTHONPATH '%(LOCALRT)s'
...
"""%env)
        ...
        if 'REFERENCEDBSCRIPTS' in env:
            create_script = os.path.join(env['REFERENCEDBSCRIPTS'],
                                         'createReferenceDb.py')
        else:
            create_script = os.path.join(env["LOCALRT"],'src',
                                         'PyCoolUtilities','tests',
...
""")
    ...
    def Run(self, context, result):
        ...
        if 'REFERENCEDBSCRIPTS' in env:
            script = os.path.join(env['REFERENCEDBSCRIPTS'],
                                  'testReferenceDbAll.py')
        else:
            script = os.path.join(env["LOCALRT"],'src',
                                  'PyCoolUtilities','tests',
