...
// Adapted from SealBase/sysapi/Time.h
#ifdef _WIN32
# include <windows.h>
#else
# include <sys/time.h>
...
namespace seal {
   ...
   *///
# define SECS_1601_TO_1970 ((369 * 365 + 89) * 86400LL)
#endif
              ...
              bool local /* = true */)
  {
    ...
    // by setting tm_isdst to -1 mktime() will try to determine whether
    //  it is daylight savings time
    // still could be present some ambiguities
    // see for example http://std.dkuug.dk/JTC1/SC22/WG14/www/docs/dr_280.htm
    val.tm_isdst = -1;
    // in case of UTC there is now summer time (set tm_isdst = 0)
    if (!local) val.tm_isdst = 0;
  ...
  {
    if (nsecpart)
      *nsecpart = static_cast<long>(m_nsecs % SEC_NSECS);

    time_t val = static_cast<time_t>(m_nsecs / SEC_NSECS);
  ...
  { return static_cast<long>(m_nsecs % SEC_NSECS); }
...
#else
    // Adapted from WINE.
    time_t utctime = static_cast<time_t>(m_nsecs / SEC_NSECS);
  ...
  Time::nanoformat (int minwidth /* = 1 */, int maxwidth /* = 9 */) const
  {
    ...
    long value = static_cast<long>(m_nsecs % SEC_NSECS);
    ASSERT (value >= 0);
    ...
    char buf [10];
    char *p = buf + 8;
    snprintf (buf, 10, "%09ld", value); // Fix Coverity SECURE_CODING
