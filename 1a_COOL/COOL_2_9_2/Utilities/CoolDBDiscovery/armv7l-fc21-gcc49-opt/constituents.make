...
#cmt_local_tagfile = $(CoolDBDiscovery_tag).make
cmt_local_tagfile = $(bin)$(CoolDBDiscovery_tag).make
...
#cmt_final_setup = $(bin)CoolDBDiscoverysetup.make
...
cmt_coolDBDiscovery_has_no_target_tag = 1
cmt_coolDBDiscovery_has_prototypes = 1

#--------------------------------------

ifdef cmt_coolDBDiscovery_has_target_tag

cmt_local_tagfile_coolDBDiscovery = $(bin)$(CoolDBDiscovery_tag)_coolDBDiscovery.make
cmt_final_setup_coolDBDiscovery = $(bin)setup_coolDBDiscovery.make
cmt_local_coolDBDiscovery_makefile = $(bin)coolDBDiscovery.make

coolDBDiscovery_extratags = -tag_add=target_coolDBDiscovery

else

cmt_local_tagfile_coolDBDiscovery = $(bin)$(CoolDBDiscovery_tag).make
cmt_final_setup_coolDBDiscovery = $(bin)setup.make
cmt_local_coolDBDiscovery_makefile = $(bin)coolDBDiscovery.make

endif

not_coolDBDiscoverycompile_dependencies = { n=0; for p in $?; do m=0; for d in $(coolDBDiscoverycompile_dependencies); do if [ $$p = $$d ]; then m=1; break; fi; done; if [ $$m -eq 0 ]; then n=1; break; fi; done; [ $$n -eq 1 ]; }

ifdef STRUCTURED_OUTPUT
coolDBDiscoverydirs :
	@if test ! -d $(bin)coolDBDiscovery; then $(mkdir) -p $(bin)coolDBDiscovery; fi
	$(echo) "STRUCTURED_OUTPUT="$(bin)coolDBDiscovery
else
coolDBDiscoverydirs : ;
endif
...
ifndef QUICK
$(cmt_local_coolDBDiscovery_makefile) : $(coolDBDiscoverycompile_dependencies) build_library_links
	$(echo) "(constituents.make) Building coolDBDiscovery.make"; \
	  $(cmtexe) -tag=$(tags) $(coolDBDiscovery_extratags) build constituent_config -out=$(cmt_local_coolDBDiscovery_makefile) coolDBDiscovery
else
$(cmt_local_coolDBDiscovery_makefile) : $(coolDBDiscoverycompile_dependencies) $(cmt_build_library_linksstamp) $(use_requirements)
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_coolDBDiscovery) ] || \
	  [ ! -f $(cmt_final_setup_coolDBDiscovery) ] || \
	  $(not_coolDBDiscoverycompile_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
	  echo "$(CMTMSGPREFIX)" "(constituents.make) Building coolDBDiscovery.make"; \
	  $(cmtexe) -tag=$(tags) $(coolDBDiscovery_extratags) build constituent_config -out=$(cmt_local_coolDBDiscovery_makefile) coolDBDiscovery; \
	  fi
endif
...
$(cmt_local_coolDBDiscovery_makefile) : $(coolDBDiscoverycompile_dependencies) build_library_links
	$(echo) "(constituents.make) Building coolDBDiscovery.make"; \
	  $(cmtexe) -f=$(bin)coolDBDiscovery.in -tag=$(tags) $(coolDBDiscovery_extratags) build constituent_makefile -without_cmt -out=$(cmt_local_coolDBDiscovery_makefile) coolDBDiscovery
else
$(cmt_local_coolDBDiscovery_makefile) : $(coolDBDiscoverycompile_dependencies) $(cmt_build_library_linksstamp) $(bin)coolDBDiscovery.in
	@if [ ! -f $@ ] || [ ! -f $(cmt_local_tagfile_coolDBDiscovery) ] || \
	  [ ! -f $(cmt_final_setup_coolDBDiscovery) ] || \
	  $(not_coolDBDiscoverycompile_dependencies) ; then \
	  test -z "$(cmtmsg)" || \
...
#	  $(cmtexe) -tag=$(tags) $(coolDBDiscovery_extratags) build constituent_makefile -out=$(cmt_local_coolDBDiscovery_makefile) coolDBDiscovery

coolDBDiscovery :: coolDBDiscoverycompile coolDBDiscoveryinstall ;

ifdef cmt_coolDBDiscovery_has_prototypes

coolDBDiscoveryprototype : $(coolDBDiscoveryprototype_dependencies) $(cmt_local_coolDBDiscovery_makefile) dirs coolDBDiscoverydirs
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_coolDBDiscovery_makefile); then \
	  $(MAKE) -f $(cmt_local_coolDBDiscovery_makefile) $@; \
	  fi
#	@$(MAKE) -f $(cmt_local_coolDBDiscovery_makefile) $@
	$(echo) "(constituents.make) $@ done"

coolDBDiscoverycompile : coolDBDiscoveryprototype

endif

coolDBDiscoverycompile : $(coolDBDiscoverycompile_dependencies) $(cmt_local_coolDBDiscovery_makefile) dirs coolDBDiscoverydirs
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_coolDBDiscovery_makefile); then \
	  $(MAKE) -f $(cmt_local_coolDBDiscovery_makefile) $@; \
	  fi
...
clean :: coolDBDiscoveryclean ;

coolDBDiscoveryclean :: $(coolDBDiscoveryclean_dependencies) ##$(cmt_local_coolDBDiscovery_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_coolDBDiscovery_makefile); then \
	  $(MAKE) -f $(cmt_local_coolDBDiscovery_makefile) $@; \
	fi
	$(echo) "(constituents.make) $@ done"
#	@-$(MAKE) -f $(cmt_local_coolDBDiscovery_makefile) coolDBDiscoveryclean

##	  /bin/rm -f $(cmt_local_coolDBDiscovery_makefile) $(bin)coolDBDiscovery_dependencies.make

install :: coolDBDiscoveryinstall ;

coolDBDiscoveryinstall :: coolDBDiscoverycompile $(coolDBDiscovery_dependencies) $(cmt_local_coolDBDiscovery_makefile)
	$(echo) "(constituents.make) Starting $@"
	@if test -f $(cmt_local_coolDBDiscovery_makefile); then \
	  $(MAKE) -f $(cmt_local_coolDBDiscovery_makefile) $@; \
	  fi
...
uninstall : coolDBDiscoveryuninstall

$(foreach d,$(coolDBDiscovery_dependencies),$(eval $(d)uninstall_dependencies += coolDBDiscoveryuninstall))

coolDBDiscoveryuninstall : $(coolDBDiscoveryuninstall_dependencies) ##$(cmt_local_coolDBDiscovery_makefile)
	$(echo) "(constituents.make) Starting $@"
	@-if test -f $(cmt_local_coolDBDiscovery_makefile); then \
	  $(MAKE) -f $(cmt_local_coolDBDiscovery_makefile) uninstall; \
	  fi
#	@$(MAKE) -f $(cmt_local_coolDBDiscovery_makefile) uninstall
	$(echo) "(constituents.make) $@ done"

remove_library_links :: coolDBDiscoveryuninstall ;

ifndef PEDANTIC
.DEFAULT::
	$(echo) "(constituents.make) Starting $@ coolDBDiscovery"
	$(echo) Using default action for $@
	$(echo) "(constituents.make) $@ coolDBDiscovery done"
endif
...
cmt_local_tagfile_install_includes = $(bin)$(CoolDBDiscovery_tag)_install_includes.make
...
cmt_local_tagfile_install_includes = $(bin)$(CoolDBDiscovery_tag).make
...
cmt_local_tagfile_install_pythonmods = $(bin)$(CoolDBDiscovery_tag)_install_pythonmods.make
...
cmt_local_tagfile_install_pythonmods = $(bin)$(CoolDBDiscovery_tag).make
...
cmt_local_tagfile_make = $(bin)$(CoolDBDiscovery_tag)_make.make
...
cmt_local_tagfile_make = $(bin)$(CoolDBDiscovery_tag).make
...
cmt_local_tagfile_tests = $(bin)$(CoolDBDiscovery_tag)_tests.make
...
cmt_local_tagfile_tests = $(bin)$(CoolDBDiscovery_tag).make
...
cmt_local_tagfile_examples = $(bin)$(CoolDBDiscovery_tag)_examples.make
...
cmt_local_tagfile_examples = $(bin)$(CoolDBDiscovery_tag).make
