...
def oracleLockTableStats( dbIdProp, tableName, debug=False ) :
    ...
    sqlFile = sqlDir  + os.sep + "oracleLockTableStats.sql"
...
def lockSingleTableStats( dbIdProp, tableName, debug=False ) :
    if tableName=="":
        print "ERROR! Table name not specified"
        sys.exit(-1)
    print "Lock statistics for table '"+tableName+"'"
    tech = CoolAuthentication.technology( dbIdProp )
    if tech == "oracle" :
        columns = oracleLockTableStats( dbIdProp, tableName, debug )
    else :
        print "ERROR! Technology '" + tech + "' is not supported"
        sys.exit(-1)
    return

def lockTableStats( dbId, tableName, debug=False ) :
    dbIdProp = CoolAuthentication.getDbIdProperties( dbId, debug )
    tech = CoolAuthentication.technology( dbIdProp )
    if debug : print "Technology: '" + tech + "'"
    if tech == "oracle" :
        if tableName=="":
            print "Lock statistics for ALL tables in database '" + dbId + "'"
            for table in showTables( dbId, debug ):
                lockSingleTableStats( dbIdProp, table, debug )
        else:
            print "Lock statistics for table '" + tableName + \
                  "' in database '" + dbId + "'"
            lockSingleTableStats( dbIdProp, tableName, debug )
    elif tech == "mysql" :
