// $Id: RelationalObjectTable.cpp,v 1.227 2013-04-22 08:33:11 avalassi Exp $
...
( const IRecordSpecification& payloadSpec, PayloadMode::Mode type )
{
  ...
  if ( type == PayloadMode::INLINEPAYLOAD ) {
  ...
  else if ( type == PayloadMode::SEPARATEPAYLOAD )
  {
 ...
 bool fetchPayload )
{
  ...
  *///
  ...
  if ( ( m_payloadMode == PayloadMode::SEPARATEPAYLOAD ||
         m_payloadMode == PayloadMode::VECTORPAYLOAD )
       && fetchPayload )
  {
    mainQuery.addFromItem( m_payloadTableName, "COOL_P5" );
    cool_payload = "COOL_P5.";
    const std::string& payloadIdName =
      m_payloadMode == PayloadMode::SEPARATEPAYLOAD ?
...

  {
    ...
    if ( m_payloadMode == PayloadMode::VECTORPAYLOAD && fetchPayload )
    {
