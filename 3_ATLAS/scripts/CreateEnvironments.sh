...
function CreatePaths {
	...
	echo "export CMTPROJECTPATH="$TopDir":"$GaudiDir/../":"$TopDir/$line/$line-$VERSION":"$LCG_install":"$ROOTDIR"" >> BuildSetup.sh
	echo "export CMTPATH="$TopDir/$line/$line-$VERSION":"$LCG_install"/LCGCMT:"$ROOTDIR"" >> BuildSetup.sh
	echo "export CPLUS_INCLUDE_PATH="$CPLUS_INCLUDE_PATH":"$GaudiDir"/InstallArea/"$CMTCONFIG"/include:"$CORAL_include":"$COOL_include":"$LCG_install"/uuid/*/"$CMTCONFIG"/include:"$LCG_install"/gperftools/2.2.1/"$CMTCONFIG"/include:"$LCG_install"/clhep/*/"$CMTCONFIG"/include:"$LCG_install"/AIDA/3.2.1/"$CMTCONFIG"/src/cpp:"$TopDir"/AtlasCore/AtlasCore-${VERSION}/External/AtlasGdb/"$CMTCONFIG"/pkg-build-install-gdb/include" >> BuildSetup.sh	
