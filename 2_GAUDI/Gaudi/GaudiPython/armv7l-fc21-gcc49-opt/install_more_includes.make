...
#cmt_local_tagfile_install_more_includes = $(GaudiPython_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(GaudiPython_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiPython_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiPythonsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "GaudiPython" = ""; then \
	    if test -d ../GaudiPython; then \
	      echo "Installing files from more ../GaudiPython to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/GaudiPython; then mkdir -p $(install_include_dir)/GaudiPython; fi;\
	        $(install_command) ../GaudiPython $(install_include_dir);\
	    else \
	      echo "No more GaudiPython include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/GaudiPython"; \
	  $(uninstall_command) "$(install_include_dir)/GaudiPython/*" ; \
	  if test -f $(install_include_dir)/GaudiPython.cmtref ; then \
	    echo "Removing $(install_include_dir)/GaudiPython.cmtref"; \
	    eval rm -f $(install_include_dir)/GaudiPython.cmtref ; \
	  fi; \
	fi
