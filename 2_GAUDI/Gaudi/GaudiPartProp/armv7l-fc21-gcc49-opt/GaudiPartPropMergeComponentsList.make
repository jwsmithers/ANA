#-- start of make_header -----------------

#====================================
#  Document GaudiPartPropMergeComponentsList
#
#   Generated Mon Feb 16 19:54:37 2015  by jwsmith
#
#====================================

include ${CMTROOT}/src/Makefile.core

ifdef tag
CMTEXTRATAGS = $(tag)
else
tag       = $(CMTCONFIG)
endif

cmt_GaudiPartPropMergeComponentsList_has_no_target_tag = 1

#--------------------------------------------------------

ifdef cmt_GaudiPartPropMergeComponentsList_has_target_tag

tags      = $(tag),$(CMTEXTRATAGS),target_GaudiPartPropMergeComponentsList

GaudiPartProp_tag = $(tag)

#cmt_local_tagfile_GaudiPartPropMergeComponentsList = $(GaudiPartProp_tag)_GaudiPartPropMergeComponentsList.make
cmt_local_tagfile_GaudiPartPropMergeComponentsList = $(bin)$(GaudiPartProp_tag)_GaudiPartPropMergeComponentsList.make

else

tags      = $(tag),$(CMTEXTRATAGS)

GaudiPartProp_tag = $(tag)

#cmt_local_tagfile_GaudiPartPropMergeComponentsList = $(GaudiPartProp_tag).make
cmt_local_tagfile_GaudiPartPropMergeComponentsList = $(bin)$(GaudiPartProp_tag).make

endif

include $(cmt_local_tagfile_GaudiPartPropMergeComponentsList)
#-include $(cmt_local_tagfile_GaudiPartPropMergeComponentsList)

ifdef cmt_GaudiPartPropMergeComponentsList_has_target_tag

cmt_final_setup_GaudiPartPropMergeComponentsList = $(bin)setup_GaudiPartPropMergeComponentsList.make
cmt_dependencies_in_GaudiPartPropMergeComponentsList = $(bin)dependencies_GaudiPartPropMergeComponentsList.in
#cmt_final_setup_GaudiPartPropMergeComponentsList = $(bin)GaudiPartProp_GaudiPartPropMergeComponentsListsetup.make
cmt_local_GaudiPartPropMergeComponentsList_makefile = $(bin)GaudiPartPropMergeComponentsList.make

else

cmt_final_setup_GaudiPartPropMergeComponentsList = $(bin)setup.make
cmt_dependencies_in_GaudiPartPropMergeComponentsList = $(bin)dependencies.in
#cmt_final_setup_GaudiPartPropMergeComponentsList = $(bin)GaudiPartPropsetup.make
cmt_local_GaudiPartPropMergeComponentsList_makefile = $(bin)GaudiPartPropMergeComponentsList.make

endif

#cmt_final_setup = $(bin)setup.make
#cmt_final_setup = $(bin)GaudiPartPropsetup.make

#GaudiPartPropMergeComponentsList :: ;

dirs ::
	@if test ! -r requirements ; then echo "No requirements file" ; fi; \
	  if test ! -d $(bin) ; then $(mkdir) -p $(bin) ; fi

javadirs ::
	@if test ! -d $(javabin) ; then $(mkdir) -p $(javabin) ; fi

srcdirs ::
	@if test ! -d $(src) ; then $(mkdir) -p $(src) ; fi

help ::
	$(echo) 'GaudiPartPropMergeComponentsList'

binobj = 
ifdef STRUCTURED_OUTPUT
binobj = GaudiPartPropMergeComponentsList/
#GaudiPartPropMergeComponentsList::
#	@if test ! -d $(bin)$(binobj) ; then $(mkdir) -p $(bin)$(binobj) ; fi
#	$(echo) "STRUCTURED_OUTPUT="$(bin)$(binobj)
endif

${CMTROOT}/src/Makefile.core : ;
ifdef use_requirements
$(use_requirements) : ;
endif

#-- end of make_header ------------------
# File: cmt/fragments/merge_componentslist_header
# Author: Sebastien Binet (binet@cern.ch)

# Makefile fragment to merge a <library>.components file into a single
# <project>.components file in the (lib) install area
# If no InstallArea is present the fragment is dummy


.PHONY: GaudiPartPropMergeComponentsList GaudiPartPropMergeComponentsListclean

# default is already '#'
#genmap_comment_char := "'#'"

componentsListRef    := ../$(tag)/GaudiPartProp.components

ifdef CMTINSTALLAREA
componentsListDir    := ${CMTINSTALLAREA}/$(tag)/lib
mergedComponentsList := $(componentsListDir)/$(project).components
stampComponentsList  := $(componentsListRef).stamp
else
componentsListDir    := ../$(tag)
mergedComponentsList :=
stampComponentsList  :=
endif

GaudiPartPropMergeComponentsList :: $(stampComponentsList) $(mergedComponentsList)
	@:

.NOTPARALLEL : $(stampComponentsList) $(mergedComponentsList)

$(stampComponentsList) $(mergedComponentsList) :: $(componentsListRef)
	@echo "Running merge_componentslist  GaudiPartPropMergeComponentsList"
	$(merge_componentslist_cmd) --do-merge \
         --input-file $(componentsListRef) --merged-file $(mergedComponentsList)

GaudiPartPropMergeComponentsListclean ::
	$(cleanup_silent) $(merge_componentslist_cmd) --un-merge \
         --input-file $(componentsListRef) --merged-file $(mergedComponentsList) ;
	$(cleanup_silent) $(remove_command) $(stampComponentsList)
libGaudiPartProp_so_dependencies = ../armv7l-fc21-gcc49-opt/libGaudiPartProp.so
#-- start of cleanup_header --------------

clean :: GaudiPartPropMergeComponentsListclean ;
#	@cd .

ifndef PEDANTIC
.DEFAULT::
	$(echo) "(GaudiPartPropMergeComponentsList.make) $@: No rule for such target" >&2
else
.DEFAULT::
	$(error PEDANTIC: $@: No rule for such target)
endif

GaudiPartPropMergeComponentsListclean ::
#-- end of cleanup_header ---------------
