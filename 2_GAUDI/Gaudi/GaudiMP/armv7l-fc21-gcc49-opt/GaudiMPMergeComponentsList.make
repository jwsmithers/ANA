#-- start of make_header -----------------

#====================================
#  Document GaudiMPMergeComponentsList
#
#   Generated Mon Feb 16 20:49:53 2015  by jwsmith
#
#====================================

include ${CMTROOT}/src/Makefile.core

ifdef tag
CMTEXTRATAGS = $(tag)
else
tag       = $(CMTCONFIG)
endif

cmt_GaudiMPMergeComponentsList_has_no_target_tag = 1

#--------------------------------------------------------

ifdef cmt_GaudiMPMergeComponentsList_has_target_tag

tags      = $(tag),$(CMTEXTRATAGS),target_GaudiMPMergeComponentsList

GaudiMP_tag = $(tag)

#cmt_local_tagfile_GaudiMPMergeComponentsList = $(GaudiMP_tag)_GaudiMPMergeComponentsList.make
cmt_local_tagfile_GaudiMPMergeComponentsList = $(bin)$(GaudiMP_tag)_GaudiMPMergeComponentsList.make

else

tags      = $(tag),$(CMTEXTRATAGS)

GaudiMP_tag = $(tag)

#cmt_local_tagfile_GaudiMPMergeComponentsList = $(GaudiMP_tag).make
cmt_local_tagfile_GaudiMPMergeComponentsList = $(bin)$(GaudiMP_tag).make

endif

include $(cmt_local_tagfile_GaudiMPMergeComponentsList)
#-include $(cmt_local_tagfile_GaudiMPMergeComponentsList)

ifdef cmt_GaudiMPMergeComponentsList_has_target_tag

cmt_final_setup_GaudiMPMergeComponentsList = $(bin)setup_GaudiMPMergeComponentsList.make
cmt_dependencies_in_GaudiMPMergeComponentsList = $(bin)dependencies_GaudiMPMergeComponentsList.in
#cmt_final_setup_GaudiMPMergeComponentsList = $(bin)GaudiMP_GaudiMPMergeComponentsListsetup.make
cmt_local_GaudiMPMergeComponentsList_makefile = $(bin)GaudiMPMergeComponentsList.make

else

cmt_final_setup_GaudiMPMergeComponentsList = $(bin)setup.make
cmt_dependencies_in_GaudiMPMergeComponentsList = $(bin)dependencies.in
#cmt_final_setup_GaudiMPMergeComponentsList = $(bin)GaudiMPsetup.make
cmt_local_GaudiMPMergeComponentsList_makefile = $(bin)GaudiMPMergeComponentsList.make

endif

#cmt_final_setup = $(bin)setup.make
#cmt_final_setup = $(bin)GaudiMPsetup.make

#GaudiMPMergeComponentsList :: ;

dirs ::
	@if test ! -r requirements ; then echo "No requirements file" ; fi; \
	  if test ! -d $(bin) ; then $(mkdir) -p $(bin) ; fi

javadirs ::
	@if test ! -d $(javabin) ; then $(mkdir) -p $(javabin) ; fi

srcdirs ::
	@if test ! -d $(src) ; then $(mkdir) -p $(src) ; fi

help ::
	$(echo) 'GaudiMPMergeComponentsList'

binobj = 
ifdef STRUCTURED_OUTPUT
binobj = GaudiMPMergeComponentsList/
#GaudiMPMergeComponentsList::
#	@if test ! -d $(bin)$(binobj) ; then $(mkdir) -p $(bin)$(binobj) ; fi
#	$(echo) "STRUCTURED_OUTPUT="$(bin)$(binobj)
endif

${CMTROOT}/src/Makefile.core : ;
ifdef use_requirements
$(use_requirements) : ;
endif

#-- end of make_header ------------------
# File: cmt/fragments/merge_componentslist_header
# Author: Sebastien Binet (binet@cern.ch)

# Makefile fragment to merge a <library>.components file into a single
# <project>.components file in the (lib) install area
# If no InstallArea is present the fragment is dummy


.PHONY: GaudiMPMergeComponentsList GaudiMPMergeComponentsListclean

# default is already '#'
#genmap_comment_char := "'#'"

componentsListRef    := ../$(tag)/GaudiMP.components

ifdef CMTINSTALLAREA
componentsListDir    := ${CMTINSTALLAREA}/$(tag)/lib
mergedComponentsList := $(componentsListDir)/$(project).components
stampComponentsList  := $(componentsListRef).stamp
else
componentsListDir    := ../$(tag)
mergedComponentsList :=
stampComponentsList  :=
endif

GaudiMPMergeComponentsList :: $(stampComponentsList) $(mergedComponentsList)
	@:

.NOTPARALLEL : $(stampComponentsList) $(mergedComponentsList)

$(stampComponentsList) $(mergedComponentsList) :: $(componentsListRef)
	@echo "Running merge_componentslist  GaudiMPMergeComponentsList"
	$(merge_componentslist_cmd) --do-merge \
         --input-file $(componentsListRef) --merged-file $(mergedComponentsList)

GaudiMPMergeComponentsListclean ::
	$(cleanup_silent) $(merge_componentslist_cmd) --un-merge \
         --input-file $(componentsListRef) --merged-file $(mergedComponentsList) ;
	$(cleanup_silent) $(remove_command) $(stampComponentsList)
libGaudiMP_so_dependencies = ../armv7l-fc21-gcc49-opt/libGaudiMP.so
#-- start of cleanup_header --------------

clean :: GaudiMPMergeComponentsListclean ;
#	@cd .

ifndef PEDANTIC
.DEFAULT::
	$(echo) "(GaudiMPMergeComponentsList.make) $@: No rule for such target" >&2
else
.DEFAULT::
	$(error PEDANTIC: $@: No rule for such target)
endif

GaudiMPMergeComponentsListclean ::
#-- end of cleanup_header ---------------
