...
#cmt_local_tagfile_install_more_includes = $(GaudiMP_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(GaudiMP_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiMP_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiMPsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "GaudiMP" = ""; then \
	    if test -d ../GaudiMP; then \
	      echo "Installing files from more ../GaudiMP to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/GaudiMP; then mkdir -p $(install_include_dir)/GaudiMP; fi;\
	        $(install_command) ../GaudiMP $(install_include_dir);\
	    else \
	      echo "No more GaudiMP include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/GaudiMP"; \
	  $(uninstall_command) "$(install_include_dir)/GaudiMP/*" ; \
	  if test -f $(install_include_dir)/GaudiMP.cmtref ; then \
	    echo "Removing $(install_include_dir)/GaudiMP.cmtref"; \
	    eval rm -f $(install_include_dir)/GaudiMP.cmtref ; \
	  fi; \
	fi
