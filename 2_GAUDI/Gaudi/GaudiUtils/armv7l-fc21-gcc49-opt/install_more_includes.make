...
#cmt_local_tagfile_install_more_includes = $(GaudiUtils_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(GaudiUtils_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiUtils_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiUtilssetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "GaudiUtils" = ""; then \
	    if test -d ../GaudiUtils; then \
	      echo "Installing files from more ../GaudiUtils to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/GaudiUtils; then mkdir -p $(install_include_dir)/GaudiUtils; fi;\
	        $(install_command) ../GaudiUtils $(install_include_dir);\
	    else \
	      echo "No more GaudiUtils include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/GaudiUtils"; \
	  $(uninstall_command) "$(install_include_dir)/GaudiUtils/*" ; \
	  if test -f $(install_include_dir)/GaudiUtils.cmtref ; then \
	    echo "Removing $(install_include_dir)/GaudiUtils.cmtref"; \
	    eval rm -f $(install_include_dir)/GaudiUtils.cmtref ; \
	  fi; \
	fi
