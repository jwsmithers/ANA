...
#cmt_local_tagfile_install_more_includes = $(GaudiAlg_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(GaudiAlg_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiAlg_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiAlgsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "GaudiAlg" = ""; then \
	    if test -d ../GaudiAlg; then \
	      echo "Installing files from more ../GaudiAlg to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/GaudiAlg; then mkdir -p $(install_include_dir)/GaudiAlg; fi;\
	        $(install_command) ../GaudiAlg $(install_include_dir);\
	    else \
	      echo "No more GaudiAlg include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/GaudiAlg"; \
	  $(uninstall_command) "$(install_include_dir)/GaudiAlg/*" ; \
	  if test -f $(install_include_dir)/GaudiAlg.cmtref ; then \
	    echo "Removing $(install_include_dir)/GaudiAlg.cmtref"; \
	    eval rm -f $(install_include_dir)/GaudiAlg.cmtref ; \
	  fi; \
	fi
