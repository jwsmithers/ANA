...
#cmt_local_tagfile_install_more_includes = $(GaudiKernel_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(GaudiKernel_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiKernel_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiKernelsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "GaudiKernel" = ""; then \
	    if test -d ../GaudiKernel; then \
	      echo "Installing files from more ../GaudiKernel to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/GaudiKernel; then mkdir -p $(install_include_dir)/GaudiKernel; fi;\
	        $(install_command) ../GaudiKernel $(install_include_dir);\
	    else \
	      echo "No more GaudiKernel include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/GaudiKernel"; \
	  $(uninstall_command) "$(install_include_dir)/GaudiKernel/*" ; \
	  if test -f $(install_include_dir)/GaudiKernel.cmtref ; then \
	    echo "Removing $(install_include_dir)/GaudiKernel.cmtref"; \
	    eval rm -f $(install_include_dir)/GaudiKernel.cmtref ; \
	  fi; \
	fi
