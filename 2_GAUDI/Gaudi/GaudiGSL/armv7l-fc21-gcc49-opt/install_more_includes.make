...
#cmt_local_tagfile_install_more_includes = $(GaudiGSL_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(GaudiGSL_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiGSL_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiGSLsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "GaudiMath" = "";then \
	    if test -d ../GaudiMath; then \
	       echo "Installing files from offset ../GaudiMath to $(install_include_dir)"; \
	       if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	       if test ! -d $(install_include_dir)/GaudiMath; then mkdir -p $(install_include_dir)/GaudiMath; fi;\
	       $(install_command) ../GaudiMath $(install_include_dir);\
	    else \
	       echo "no offset  GaudiMath include directory"; \
	    fi; \
	  fi; \
	  if test ! "GaudiGSL" = ""; then \
	    if test -d ../GaudiGSL; then \
	      echo "Installing files from more ../GaudiGSL to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/GaudiGSL; then mkdir -p $(install_include_dir)/GaudiGSL; fi;\
	        $(install_command) ../GaudiGSL $(install_include_dir);\
	    else \
	      echo "No more GaudiGSL include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/GaudiGSL"; \
	  $(uninstall_command) "$(install_include_dir)/GaudiGSL/*" ; \
	  if test -f $(install_include_dir)/GaudiGSL.cmtref ; then \
	    echo "Removing $(install_include_dir)/GaudiGSL.cmtref"; \
	    eval rm -f $(install_include_dir)/GaudiGSL.cmtref ; \
	  fi; \
	fi
