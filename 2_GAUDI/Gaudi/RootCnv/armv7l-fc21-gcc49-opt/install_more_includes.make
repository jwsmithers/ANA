...
#cmt_local_tagfile_install_more_includes = $(RootCnv_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(RootCnv_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)RootCnv_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)RootCnvsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "RootCnv" = ""; then \
	    if test -d ../RootCnv; then \
	      echo "Installing files from more ../RootCnv to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/RootCnv; then mkdir -p $(install_include_dir)/RootCnv; fi;\
	        $(install_command) ../RootCnv $(install_include_dir);\
	    else \
	      echo "No more RootCnv include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/RootCnv"; \
	  $(uninstall_command) "$(install_include_dir)/RootCnv/*" ; \
	  if test -f $(install_include_dir)/RootCnv.cmtref ; then \
	    echo "Removing $(install_include_dir)/RootCnv.cmtref"; \
	    eval rm -f $(install_include_dir)/RootCnv.cmtref ; \
	  fi; \
	fi
