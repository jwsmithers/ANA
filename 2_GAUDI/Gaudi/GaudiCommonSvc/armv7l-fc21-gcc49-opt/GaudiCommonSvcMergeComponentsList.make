#-- start of make_header -----------------

#====================================
#  Document GaudiCommonSvcMergeComponentsList
#
#   Generated Mon Feb 16 20:23:11 2015  by jwsmith
#
#====================================

include ${CMTROOT}/src/Makefile.core

ifdef tag
CMTEXTRATAGS = $(tag)
else
tag       = $(CMTCONFIG)
endif

cmt_GaudiCommonSvcMergeComponentsList_has_no_target_tag = 1

#--------------------------------------------------------

ifdef cmt_GaudiCommonSvcMergeComponentsList_has_target_tag

tags      = $(tag),$(CMTEXTRATAGS),target_GaudiCommonSvcMergeComponentsList

GaudiCommonSvc_tag = $(tag)

#cmt_local_tagfile_GaudiCommonSvcMergeComponentsList = $(GaudiCommonSvc_tag)_GaudiCommonSvcMergeComponentsList.make
cmt_local_tagfile_GaudiCommonSvcMergeComponentsList = $(bin)$(GaudiCommonSvc_tag)_GaudiCommonSvcMergeComponentsList.make

else

tags      = $(tag),$(CMTEXTRATAGS)

GaudiCommonSvc_tag = $(tag)

#cmt_local_tagfile_GaudiCommonSvcMergeComponentsList = $(GaudiCommonSvc_tag).make
cmt_local_tagfile_GaudiCommonSvcMergeComponentsList = $(bin)$(GaudiCommonSvc_tag).make

endif

include $(cmt_local_tagfile_GaudiCommonSvcMergeComponentsList)
#-include $(cmt_local_tagfile_GaudiCommonSvcMergeComponentsList)

ifdef cmt_GaudiCommonSvcMergeComponentsList_has_target_tag

cmt_final_setup_GaudiCommonSvcMergeComponentsList = $(bin)setup_GaudiCommonSvcMergeComponentsList.make
cmt_dependencies_in_GaudiCommonSvcMergeComponentsList = $(bin)dependencies_GaudiCommonSvcMergeComponentsList.in
#cmt_final_setup_GaudiCommonSvcMergeComponentsList = $(bin)GaudiCommonSvc_GaudiCommonSvcMergeComponentsListsetup.make
cmt_local_GaudiCommonSvcMergeComponentsList_makefile = $(bin)GaudiCommonSvcMergeComponentsList.make

else

cmt_final_setup_GaudiCommonSvcMergeComponentsList = $(bin)setup.make
cmt_dependencies_in_GaudiCommonSvcMergeComponentsList = $(bin)dependencies.in
#cmt_final_setup_GaudiCommonSvcMergeComponentsList = $(bin)GaudiCommonSvcsetup.make
cmt_local_GaudiCommonSvcMergeComponentsList_makefile = $(bin)GaudiCommonSvcMergeComponentsList.make

endif

#cmt_final_setup = $(bin)setup.make
#cmt_final_setup = $(bin)GaudiCommonSvcsetup.make

#GaudiCommonSvcMergeComponentsList :: ;

dirs ::
	@if test ! -r requirements ; then echo "No requirements file" ; fi; \
	  if test ! -d $(bin) ; then $(mkdir) -p $(bin) ; fi

javadirs ::
	@if test ! -d $(javabin) ; then $(mkdir) -p $(javabin) ; fi

srcdirs ::
	@if test ! -d $(src) ; then $(mkdir) -p $(src) ; fi

help ::
	$(echo) 'GaudiCommonSvcMergeComponentsList'

binobj = 
ifdef STRUCTURED_OUTPUT
binobj = GaudiCommonSvcMergeComponentsList/
#GaudiCommonSvcMergeComponentsList::
#	@if test ! -d $(bin)$(binobj) ; then $(mkdir) -p $(bin)$(binobj) ; fi
#	$(echo) "STRUCTURED_OUTPUT="$(bin)$(binobj)
endif

${CMTROOT}/src/Makefile.core : ;
ifdef use_requirements
$(use_requirements) : ;
endif

#-- end of make_header ------------------
# File: cmt/fragments/merge_componentslist_header
# Author: Sebastien Binet (binet@cern.ch)

# Makefile fragment to merge a <library>.components file into a single
# <project>.components file in the (lib) install area
# If no InstallArea is present the fragment is dummy


.PHONY: GaudiCommonSvcMergeComponentsList GaudiCommonSvcMergeComponentsListclean

# default is already '#'
#genmap_comment_char := "'#'"

componentsListRef    := ../$(tag)/GaudiCommonSvc.components

ifdef CMTINSTALLAREA
componentsListDir    := ${CMTINSTALLAREA}/$(tag)/lib
mergedComponentsList := $(componentsListDir)/$(project).components
stampComponentsList  := $(componentsListRef).stamp
else
componentsListDir    := ../$(tag)
mergedComponentsList :=
stampComponentsList  :=
endif

GaudiCommonSvcMergeComponentsList :: $(stampComponentsList) $(mergedComponentsList)
	@:

.NOTPARALLEL : $(stampComponentsList) $(mergedComponentsList)

$(stampComponentsList) $(mergedComponentsList) :: $(componentsListRef)
	@echo "Running merge_componentslist  GaudiCommonSvcMergeComponentsList"
	$(merge_componentslist_cmd) --do-merge \
         --input-file $(componentsListRef) --merged-file $(mergedComponentsList)

GaudiCommonSvcMergeComponentsListclean ::
	$(cleanup_silent) $(merge_componentslist_cmd) --un-merge \
         --input-file $(componentsListRef) --merged-file $(mergedComponentsList) ;
	$(cleanup_silent) $(remove_command) $(stampComponentsList)
libGaudiCommonSvc_so_dependencies = ../armv7l-fc21-gcc49-opt/libGaudiCommonSvc.so
#-- start of cleanup_header --------------

clean :: GaudiCommonSvcMergeComponentsListclean ;
#	@cd .

ifndef PEDANTIC
.DEFAULT::
	$(echo) "(GaudiCommonSvcMergeComponentsList.make) $@: No rule for such target" >&2
else
.DEFAULT::
	$(error PEDANTIC: $@: No rule for such target)
endif

GaudiCommonSvcMergeComponentsListclean ::
#-- end of cleanup_header ---------------
