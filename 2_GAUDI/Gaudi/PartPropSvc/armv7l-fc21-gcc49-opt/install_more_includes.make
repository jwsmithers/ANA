...
#cmt_local_tagfile_install_more_includes = $(PartPropSvc_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(PartPropSvc_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)PartPropSvc_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)PartPropSvcsetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "PartPropSvc" = ""; then \
	    if test -d ../PartPropSvc; then \
	      echo "Installing files from more ../PartPropSvc to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/PartPropSvc; then mkdir -p $(install_include_dir)/PartPropSvc; fi;\
	        $(install_command) ../PartPropSvc $(install_include_dir);\
	    else \
	      echo "No more PartPropSvc include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/PartPropSvc"; \
	  $(uninstall_command) "$(install_include_dir)/PartPropSvc/*" ; \
	  if test -f $(install_include_dir)/PartPropSvc.cmtref ; then \
	    echo "Removing $(install_include_dir)/PartPropSvc.cmtref"; \
	    eval rm -f $(install_include_dir)/PartPropSvc.cmtref ; \
	  fi; \
	fi
