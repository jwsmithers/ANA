...
#cmt_local_tagfile_install_more_includes = $(GaudiExamples_tag)_install_more_includes.make
...
#cmt_local_tagfile_install_more_includes = $(GaudiExamples_tag).make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiExamples_install_more_includessetup.make
...
#cmt_final_setup_install_more_includes = $(bin)GaudiExamplessetup.make
cmt_local_install_more_includes_makefile = $(bin)install_more_includes.make
...
$(install_include_dir) ::
	@if test "$(installarea)" = ""; then \
	  ...
	  if test ! "GaudiExamples" = ""; then \
	    if test -d ../GaudiExamples; then \
	      echo "Installing files from more ../GaudiExamples to $(install_include_dir)"; \
	      if test ! -d $(install_include_dir) ; then mkdir -p $(install_include_dir); fi; \
	      if test ! -d $(install_include_dir)/GaudiExamples; then mkdir -p $(install_include_dir)/GaudiExamples; fi;\
	        $(install_command) ../GaudiExamples $(install_include_dir);\
	    else \
	      echo "No more GaudiExamples include directory"; \
	    fi; \
          fi; \
	fi
...
install_more_includesuninstall ::
	@if test "$(installarea)" = ""; then \
	  ...
	  echo "Uninstalling files from $(install_include_dir)/GaudiExamples"; \
	  $(uninstall_command) "$(install_include_dir)/GaudiExamples/*" ; \
	  if test -f $(install_include_dir)/GaudiExamples.cmtref ; then \
	    echo "Removing $(install_include_dir)/GaudiExamples.cmtref"; \
	    eval rm -f $(install_include_dir)/GaudiExamples.cmtref ; \
	  fi; \
	fi
