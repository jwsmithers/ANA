...
coral::FrontierAccess::View::View( boost::shared_ptr<const SessionProperties> sessionProperties, const std::string& viewName )
...
{
  throw coral::InvalidOperationInReadOnlyModeException( m_sessionProperties->domainServiceName(), "FrontierAccess::View::privilegeManager" );
}
...
void coral::FrontierAccess::View::refreshDefinition()
{
  ...
  if ( ! statement.execute( coral::AttributeList(), false ) )
    throw coral::SchemaException( m_sessionProperties->domainServiceName(), "Could not read the definition of a view", "View::refreshDefinition" );
