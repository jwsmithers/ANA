  ...
  , m_schemas()
{
  ...
  {
    // Retrieve and return the Oracle server version (bug #103685)
    coral::FrontierAccess::Statement query( m_sessionProperties, "SELECT VERSION FROM PRODUCT_COMPONENT_VERSION WHERE PRODUCT LIKE 'Oracle Database%'" );
    if( ! query.execute( coral::AttributeList(), false ) )
...
coral::ISchema& coral::FrontierAccess::Session::schema( const std::string& schema )
{
  ...
  coral::FrontierAccess::Statement query( m_sessionProperties, "SELECT USERNAME FROM ALL_USERS WHERE USERNAME=:\"user\"" );
