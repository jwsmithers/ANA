...
// Random query? (SELECT 'XYZ' AS RANDOM, ...)
#undef RANDOM_QUERY
//#define RANDOM_QUERY 1
...
namespace coral
{
  ...
  public:
    ...
    static const coral::AttributeList& referenceData( int index, bool skipNaN, const std::string& randomString = "RANDOM" )
    {
    ...
    void readReferenceData( const std::string& connectString )
    {
      ...
      /*
      // For Frontier stress tests:
      // - determine the Squid URL to refresh/bypass the server-side cache
      // - modify the client URL to avoid the client-side Frontier cache
      std::string webCacheUrl = ""; // != "" only for Frontier stress test
      if ( !optDoFill() && connectString == BuildUrl( "Frontier", true ) )
      {
        ...
        if ( webCacheUrl.find("frontier") == std::string::npos )
          throw std::runtime_error( "Replica " + webCacheUrl + " for " + connectStringNew + " is not a frontier replica?" );
        if ( webCacheUrl.find(":8000/") != std::string::npos )
          throw std::runtime_error( "Replica " + webCacheUrl + " for " + connectStringNew + " is not a Squid replica?" );
        ...
        //std::cout << "webCacheUrl=" << webCacheUrl << std::endl;
        connectStringNew = "frontier://(serverurl=" + webCacheUrl + ")";
        connectStringNew += "(clientcachemaxresultsize=0)";
        connectStringNew += "(forcereload=long)"; // Only for :8000 ??
        connectStringNew += schema;
        std::cout << "Frontier stress test: "
                  << "connect to " << connectString
                  << ", refresh " << webCacheUrl << std::endl;
      }
      *///
      ...
      for ( unsigned iRead=0; iRead<optNReads(); iRead++ )
      {
        ...
        std::string freshKey = fresh.str();
        //std::cout << "freshKey=" << freshKey << std::endl;
        /*
        if ( webCacheUrl != "" )
        {
          connSvc.webCacheControl().refreshSchemaInfo( webCacheUrl );
          // See http://frontier.cern.ch/dist/FrontierClientUsage.html
          setenv( "FRONTIER_FRESHKEY", fresh.str().c_str(), 1 );
        }
        *///
#endif
        ...
        for ( int iRow = 0; iRow<=259; iRow++ )
        {
          ...
          if ( optDoFill() ) // functional test (not the stress test)
          {
    ...
    void test_oneBackend()
    {
      ...
      if ( optDoFill() ) fillReferenceData( connectStringW );
