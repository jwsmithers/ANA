...
if [ "$cout" == "stdout" ]; then
  ...
  valfile=/tmp/$USER/${baselog}_valgrind.txt
...
if [ "$doValgrind" == "1" ]; then
  echo "Kill any running instance of valgrind using coralServer and sleep 1s" >> $outfile
  ###killall -9 valgrind >& /dev/null ### DOES NOT WORK
  kill -9 `ps -ef | grep $USER | grep valgrind | grep coralServer | grep -v /usr/bin/time | grep -v grep | awk '{print $2}'` >& /dev/null
  sleep 1
...
if [ "$test" == "HLT" ] && [ "$lcg" != "cwd" ] && [ "$CMTCONFIG" != "i686-slc5-gcc43-dbg" ] && [ "$CMTCONFIG" != "i686-slc5-gcc43-opt" ]; then export CMTCONFIG=i686-slc5-gcc43-dbg; fi
...
if [ "$doValgrind" == "1" ]; then
  ###/usr/bin/time -ao/dev/stdout -f"=== TIMER (SERVER) %Uuser %Ssystem %eelapsed %PCPU" valgrind -v --leak-check=full --show-reachable=yes --error-limit=no --log-file=$valfile --suppressions=$release/src/config/qmtest/valgrind.supp --gen-suppressions=all --num-callers=50 coralServer -p 40007 $msglevel $msgformat $rotrans $monitoring >> $outfile 
  /usr/bin/time -ao/dev/stdout -f"=== TIMER (SERVER) %Uuser %Ssystem %eelapsed %PCPU" valgrind -v --leak-check=full --show-reachable=yes --error-limit=no --log-file=$valfile --suppressions=$release/src/config/qmtest/valgrind.supp --gen-suppressions=all --num-callers=50 --track-origins=yes coralServer -p 40007 $msglevel $msgformat $rotrans $monitoring >> $outfile 
else
...
# Normalise the server valgrind log
if [ "$doValgrind" == "1" ]; then
  $release/src/config/qmtest/sedValgrindLog.sh $valfile
fi
