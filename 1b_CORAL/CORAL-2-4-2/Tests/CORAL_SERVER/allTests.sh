...
if [ $test == "CoralAccess" ] || [ $test == "CoolRegression" ]; then
  ...
  ###  echo "ERROR! LCGCMT release 62 is not supported"
  ###  exit 1
  ###elif [ $lcgseries == 63 ]; then
  ###  echo "ERROR! LCGCMT release 63 is not supported"
  ###  exit 1
  ###elif [ $lcgseries == 64 ]; then
  ###  echo "ERROR! LCGCMT release 64 is not supported"
  ###  exit 1
  ###elif [ $lcgseries == 65 ]; then
  ###  REL=17.7.90
  ###elif [[ $lcgseries > 65 ]]; then
  ###  echo "ERROR! LCGCMT release > 65 is not yet supported"
  ###  exit 1
  elif [[ $lcgseries > 61 ]]; then
    echo "WARNING! $lcg is not yet supported: try LCGCMT_61 settings..."
    REL=17.1.0 # ~OK Oracle, FAILS CoralServer (boost mismatch, bug #90431/7)
  fi
  # Setup runtime environment from AtlasSetup (HLT)
  # NB Will always reset CMTCONFIG to i686-slc5-gcc43-opt
  pushd $test > /dev/null
