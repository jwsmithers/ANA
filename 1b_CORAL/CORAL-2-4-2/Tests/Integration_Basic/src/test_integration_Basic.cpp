...
namespace coral
{
  ...
  class TestIntegrationBasic : public CoralCppUnitDBTest
  {
    CPPUNIT_TEST_SUITE( TestIntegrationBasic );
  ...
  public:
    ...
    void GroupApp_readData();
    void GroupApp_fillData();
    void Transaction_run();
    void SetOperations_run();
    void InsertSelect_run();
    void Queries_run();
    void SchemaDefinition_prepareSchema();
    void SchemaDefinition_readSchema();
    void SchemaDefinition_printViewInfo( const coral::IView& view ) const;
    void SchemaDefinition_printTableInfo( const coral::ITableDescription& description ) const;
    void DmlOperations_atomicOperations();
    void DmlOperations_bulkOperations();

    void testAll(); // WAS main of previous implementation

    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

    TestIntegrationBasic(){}
    ...
    void setUp()
    {
      T1 = BuildUniqueTableName( "BASIC_T1" );
      T2 = BuildUniqueTableName( "BASIC_T2" );
      T3 = BuildUniqueTableName( "BASIC_T3" );
      T4 = BuildUniqueTableName( "BASIC_T4" );
      V0 = BuildUniqueTableName( "BASIC_V0" );
    }
  ...
  CPPUNIT_TEST_SUITE_REGISTRATION( TestIntegrationBasic );
...
void
coral::TestIntegrationBasic::GroupApp_fillData()
{
  std::auto_ptr<coral::ISessionProxy> sessionPtr( connSvc().connect( UrlRW(), coral::Update ) );
  coral::ISessionProxy& session = *sessionPtr;
...
void
coral::TestIntegrationBasic::GroupApp_readData()
{
  std::auto_ptr<coral::ISessionProxy> sessionPtr( connSvc().connect( UrlRW(), coral::Update ) );
  coral::ISessionProxy& session = *sessionPtr;
...
void
coral::TestIntegrationBasic::Transaction_run()
{
  std::auto_ptr<coral::ISessionProxy> session01( connSvc().connect( UrlRW(), coral::ReadOnly ) );
  ...
  std::auto_ptr<coral::ISessionProxy> session02( connSvc().connect( UrlRW(), coral::Update ) );
...
void
coral::TestIntegrationBasic::SetOperations_run()
{
  std::auto_ptr<coral::ISessionProxy> sessionPtr( connSvc().connect( UrlRW(), coral::Update ) );
  coral::ISessionProxy& session = *sessionPtr;
  ...
  std::string url( UrlRO() );
  std::cout << "URL: '" << url << "', Backend '" << url.substr(6, 6) << "'" << std::endl;
  if( url.substr(6, 6).compare("Oracle") == 0 )
  {
...
void
coral::TestIntegrationBasic::InsertSelect_run()
{
  std::auto_ptr<coral::ISessionProxy> sessionPtr( connSvc().connect( UrlRW(), coral::Update ) );
  coral::ISessionProxy& session = *sessionPtr;
...
void
coral::TestIntegrationBasic::Queries_run()
{
  std::auto_ptr<coral::ISessionProxy> sessionPtr( connSvc().connect( UrlRW(), coral::Update ) );
  coral::ISessionProxy& session = *sessionPtr;
  ...
  while ( cursor0.next() ) {
    ...
    long expectedBlobSize = 1000 * ( i + 1 );
    ...
    for ( long j = 0; j < expectedBlobSize; ++j, ++p )
    {
      if ( *p != ( i + j )%256 )
      {
        delete query0;
...
void
coral::TestIntegrationBasic::SchemaDefinition_prepareSchema()
{
  std::auto_ptr<coral::ISessionProxy> sessionPtr( connSvc().connect( UrlRW(), coral::Update ) );
  coral::ISessionProxy& session = *sessionPtr;
  ...
  schema.dropIfExistsTable(T4);
  schema.dropIfExistsTable(T3);
  schema.dropIfExistsTable(T2);
  schema.dropIfExistsTable(T1);

  // You should drop the view first, but dropping it last will make it invalid,
  // triggering a test for bug #92418 (ORA-24372)
  ...
  // Create a view
  IViewFactory* factory = schema.viewFactory();
  ...
  sleepSeconds( 1 ); // Avoid ORA-01466
}

//----------------------------------------------------------------------------

void
coral::TestIntegrationBasic::SchemaDefinition_printViewInfo( const coral::IView& view ) const
{
...
void
coral::TestIntegrationBasic::SchemaDefinition_readSchema()
{
  std::auto_ptr<coral::ISessionProxy> sessionPtr( connSvc().connect( UrlRW(), coral::Update ) );
  coral::ISessionProxy& session = *sessionPtr;
  ...
  // Examine the tables
  SchemaDefinition_printTableInfo( session.nominalSchema().tableHandle(T1).description() );
  SchemaDefinition_printTableInfo( session.nominalSchema().tableHandle(T2).description() );
  SchemaDefinition_printTableInfo( session.nominalSchema().tableHandle(T3).description() );

  std::cout << "read view" << std::endl;

  SchemaDefinition_printViewInfo( session.nominalSchema().viewHandle(V0) );
...
void
coral::TestIntegrationBasic::SchemaDefinition_printTableInfo( const coral::ITableDescription& description ) const
{
...
void
coral::TestIntegrationBasic::DmlOperations_atomicOperations()
{
  std::auto_ptr<coral::ISessionProxy> sessionPtr( connSvc().connect( UrlRW(), coral::Update ) );
  coral::ISessionProxy& session = *sessionPtr;
...
void
coral::TestIntegrationBasic::DmlOperations_bulkOperations()
{
  std::auto_ptr<coral::ISessionProxy> sessionPtr( connSvc().connect( UrlRW(), coral::Update ) );
  coral::ISessionProxy& session = *sessionPtr;
...
void coral::TestIntegrationBasic::testAll() // WAS main
{
  std::cout << "Hallo testAll" << std::endl;

  // No need for a try block, cppunit will catch and handle any exception
  std::cout << "Test 1: defining and reading back tables and views" << std::endl;
  //create schema and view
  SchemaDefinition_prepareSchema();

  std::cout << "Test 2: DML operations" << std::endl;
  DmlOperations_atomicOperations();
  DmlOperations_bulkOperations();

  std::cout << "Test 3: Read Only Part, SchemaDefinition, Queries" << std::endl;
  SchemaDefinition_readSchema();
  Queries_run();
  ...
  std::cout << "Goodbye testAll" << std::endl;
}

//----------------------------------------------------------------------------

CORALCPPUNITTEST_MAIN( TestIntegrationBasic )
