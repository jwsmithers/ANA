...
{
  if ( this == &anOpt ) return *this; // Fix Coverity SELF_ASSIGN
  type = anOpt.type;
  flag = anOpt.flag;
  name = anOpt.name;
  helpEntry = anOpt.helpEntry;
...
{
  for( std::vector<Option*>::const_iterator iOpt=m_options.begin(); iOpt!=m_options.end(); ++iOpt)
  {
    delete *iOpt;
  }
  for( std::vector<Command*>::const_iterator iCmd=m_commands.begin(); iCmd!=m_commands.end(); ++iCmd)
  {
    delete *iCmd;
  }
}
...
bool
coral::CommandLine::parse(int argc, char** argv, std::map<std::string,std::string>& userOptions)
{
                             ...
                             const std::map<std::string,Option*>& supportedOptions)
{
  ...
  for(std::map<std::string,std::string>::const_iterator map_iter = userOptions.begin(); map_iter!=userOptions.end(); map_iter++)
  {
  ...
  for(std::map<std::string,Option*>::const_iterator reqIOpt=supportedOptions.begin(); reqIOpt!=supportedOptions.end(); reqIOpt++)
  {
...
coral::CommandLine::parse(int argc,char** argv)
{
  ...
  if(commandMode)
  {
    ...
    Command* firstCmd = 0;
    Command* secondCmd = 0;
    for(std::map<std::string,std::string>::const_iterator uOpt = userOps.begin(); uOpt!=userOps.end(); ++uOpt)
    {
    ...
    else
    {
      ...
      else
      {
        ...
        for( std::vector<Option*>::const_iterator iO=options.begin(); iO!=options.end(); ++iO)
        {
          delete *iO;
        }
      }
                                      ...
                                      std::ostream& out) const
{
  ...
  for(std::map<std::string,bool>::const_iterator iOp=cmd.dependentOptions().begin(); iOp!=cmd.dependentOptions().end(); ++iOp)
  {
    std::string optKey("--");
