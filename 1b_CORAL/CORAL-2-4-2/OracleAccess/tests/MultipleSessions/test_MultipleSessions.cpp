...
namespace
{
  ...
  public:
    ...
    void operator()()
    {
      ...
      {
        // Write data
        std::auto_ptr<coral::ISessionProxy> session( m_connscv.connect( coralConnStringAdmin ) );
      ...
      {
        // Fix crash (bug #94505)
        std::cout << "Exception caught in thread no " << m_threadID
                  << ": " << e.what() << std::endl;
        m_nErrors++;
      }
    }
...
namespace coral
{
  ...
  public:
    ...
    test_Read()
    {
      ...
      std::auto_ptr<coral::ISessionProxy> session( connSvc.connect( coralConnStringAdmin ) );
      session->transaction().start();
      session->transaction().commit();
#endif
      ...
      for ( int i = 0; i < 10; ++i )
        threadBodies.push_back( new ThreadBody( connSvc, i, T1, nErrors ) );
      ...
      CPPUNIT_ASSERT_EQUAL_MESSAGE( "#errors", 0u, nErrors );
    }
