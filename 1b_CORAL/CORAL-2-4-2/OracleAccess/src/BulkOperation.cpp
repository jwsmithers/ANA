  ...
  m_statementStatistics( 0 )
{
  ...
  std::cout << "Create BulkOperation " << this << std::endl; // debug bug #98736
#endif
  ...
  for ( coral::AttributeList::const_iterator iAttribute = m_inputBuffer.begin();
    ...
    if ( attributeType == typeid(float) ) {
      m_dataCache.push_back( new coral::OracleAccess::PolymorphicTVector<float>( m_rowsInCache, m_sessionProperties->majorServerVersion() ) );
    }
    else if ( attributeType == typeid(double) ) {
      m_dataCache.push_back( new coral::OracleAccess::PolymorphicTVector<double>( m_rowsInCache, m_sessionProperties->majorServerVersion() ) );
    }
    else if ( attributeType == typeid(long double) ) {
      m_dataCache.push_back( new coral::OracleAccess::PolymorphicTVector<long double>( m_rowsInCache, m_sessionProperties->majorServerVersion() ) );
    }
      ...
      m_dataCache.push_back( new coral::OracleAccess::PolymorphicTVector<long long>( m_rowsInCache, m_sessionProperties ) );
    }
    else if ( attributeType == typeid(unsigned long long) ) {
      m_dataCache.push_back( new coral::OracleAccess::PolymorphicTVector<unsigned long long>( m_rowsInCache, m_sessionProperties ) );
    }
      ...
      m_dataCache.push_back( new coral::OracleAccess::PolymorphicTVector<coral::Blob>( m_rowsInCache, m_sessionProperties ) );
    }
      ...
      m_dataCache.push_back( new coral::OracleAccess::PolymorphicTVector<coral::TimeStamp>( m_rowsInCache, m_sessionProperties ) );
    }
    else {
...
{
#ifdef ORACLE_CONNECTION_PROPERTIES_DEBUG
  std::cout << "Delete BulkOperation " << this << std::endl; // debug bug #98736
#endif
  this->reset();
}
                        ...
                        dvoid** indpp )
{
  ...
  if ( !dataVector ) // Fix Coverity FORWARD_NULL
    throw coral::DataEditorException( "coral::OracleAccess", "Could not dynamic cast to PolymorphicTVector", "BindCallBackForStrings" );
...
{
  //if ( ! m_sessionProperties->isTransactionActive() ) // fix bug #91585?
  //  throw coral::TransactionNotActiveException( m_sessionProperties->domainServiceName(), "BulkOperation::flush" );
...
coral::OracleAccess::BulkOperation::reset()
{
  ...
  if ( m_ociStmtHandle ) {
    ...
    OCIStmtRelease( m_ociStmtHandle, m_sessionProperties->ociErrorHandle(), 0, 0, OCI_DEFAULT );
    m_ociStmtHandle = 0;
  }
