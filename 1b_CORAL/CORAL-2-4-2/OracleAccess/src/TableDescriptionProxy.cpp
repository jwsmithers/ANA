  ...
  m_mutex()
{
  ...
  std::cout << "Create TableDescriptionProxy " << this << std::endl; // debug bug #98736
#endif
...
{
#ifdef ORACLE_CONNECTION_PROPERTIES_DEBUG
  std::cout << "Delete TableDescriptionProxy " << this << std::endl; // debug bug #98736
#endif
...
coral::OracleAccess::TableDescriptionProxy::refreshForeignKeyInfo() const
{
  ...
  if ( m_sessionProperties->selectAnyTable() )
  {
    ...
    os << "select con.name as FK_NAME, ro.name as REF_TABLE, "
       ...
       << "from sys." << userDollar << " u, sys.obj$ o, sys.cdef$ cd, "
