// $Id: QueryDefinition.cpp,v 1.17 2012-03-06 16:56:49 avalassi Exp $
...
coral::MySQLAccess::QueryDefinition::QueryDefinition( boost::shared_ptr<const SessionProperties> properties ) :
...
coral::MySQLAccess::QueryDefinition::QueryDefinition( boost::shared_ptr<const SessionProperties> properties, const std::string& tableName ) :
  m_properties( properties ),
  m_singleTable( true ),
  m_sqlFragmnent( "" ),
  m_inputData( 0 ),
...
boost::shared_ptr<const coral::MySQLAccess::SessionProperties> coral::MySQLAccess::QueryDefinition::sessionProperties() const
{
  return m_properties;
}
...
{
  if ( ! m_sqlFragmnent.empty() )
    throw coral::QueryExecutedException( m_properties->domainServiceName(), "IQueryDefinition::setDistinct" );
  m_distinct = true;
}
...
{
  if ( ! m_sqlFragmnent.empty() )
    throw coral::QueryExecutedException( m_properties->domainServiceName(), "IQueryDefinition::addToOutputList" );
...
{
  if ( ! m_sqlFragmnent.empty() )
    throw coral::QueryExecutedException( m_properties->domainServiceName(), "IQueryDefinition::addToTableList" );
  if ( m_singleTable )
    throw coral::QueryException( m_properties->domainServiceName(), "method cannot be invoked on queries on a single table", "IQueryDefinition::addToTableList" );
...
{
  if ( ! m_sqlFragmnent.empty() )
    throw coral::QueryExecutedException( m_properties->domainServiceName(),
...
{
  if ( ! m_sqlFragmnent.empty() )
    throw coral::QueryExecutedException( m_properties->domainServiceName(), "IQueryDefinition::setCondition" );
...
{
  if ( ! m_sqlFragmnent.empty() )
    throw coral::QueryExecutedException( m_properties->domainServiceName(), "IQueryDefinition::addToOrderList" );
...
{
  if ( ! m_sqlFragmnent.empty() )
    throw coral::QueryExecutedException( m_properties->domainServiceName(), "IQueryDefinition::limitReturnedRows" );
  m_rowLimit = maxRows;
  m_rowOffset = offset;
}
...
{
  if ( ! m_sqlFragmnent.empty() )
    throw coral::QueryExecutedException( m_properties->domainServiceName(), "IQueryDefinition::applySetOperation" );
