// $Id: TableSchemaLoader.cpp,v 1.32 2013-01-14 17:35:44 avalassi Exp $
...
#ifdef CORAL240SO
#include "CoralCommon/StringOps.h"
#else
...
namespace coral
{
  ...
  {
    TableSchemaLoader::TableSchemaLoader( coral::TableDescription& description, boost::shared_ptr<const SessionProperties> sessionProperties, ISchemaProperties& schemaProperties )
    ...
    void TableSchemaLoader::loadTableColumnsDescriptions()
    {
      ...
      if( mysql_query( m_sessionProperties->connectionHandle(), fields_sql.c_str() ) )
      {
        ...
        throw coral::SessionException( errorHandler.message(), "MySQLAccess::TableSchemaLoader", m_sessionProperties->domainServiceName() );
      }
    ...
    FKConstraints TableSchemaLoader::parseFKConstraints( const std::string& tableDDL )
    {
      ...
      *///
      ...
      for( size_t i = 0; i < sll; i++ )
      {
        ...
        {
          /// Comment out to fix Coverity DEADCODE: collectPk is always false!
          /*
