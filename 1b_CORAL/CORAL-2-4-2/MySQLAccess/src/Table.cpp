// $Id: Table.cpp,v 1.15 2012-06-26 14:37:08 avalassi Exp $
...
namespace coral
{
  ...
  {
    Table::Table( const std::string& tableName, boost::shared_ptr<const SessionProperties> sessionProperties, ISchemaProperties& schemaProperties )
      ...
      , m_tablePrivilegeManager( 0 )
    {
      ...
      else delete newDescription;  // Fix memory leak (Coverity RESOURCE_LEAK)
    ...
    ITableDataEditor& Table::dataEditor()
    {
      ...
      if ( m_sessionProperties->isTransactionReadOnly() )
        throw coral::InvalidOperationInReadOnlyTransactionException( m_sessionProperties->domainServiceName(), "ITable::dataEditor" );
