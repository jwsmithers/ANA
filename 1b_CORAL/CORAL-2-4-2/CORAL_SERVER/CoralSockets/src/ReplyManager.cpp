// $Id: ReplyManager.cpp,v 1.12.2.2.2.5 2013-02-08 09:40:08 avalassi Exp $
...
boost::shared_ptr<ByteBuffer> ReplySlot::nextReply( bool& lastReply )
{
  ...
  while ( m_replies.empty() && m_status == OK )
  {
    ...
    if ( m_cond.wait_for (lock, std::chrono::milliseconds ( 20000 ) ) == std::cv_status::timeout )
#else
    if ( !m_cond.timed_wait( lock, boost::posix_time::milliseconds( 20000 ) ) )
#endif
    {
...
bool SocketReplyIterator::nextBuffer()
{
  ...
  DEBUG( "SocketReplyIterator::nextBuffer() request #"<<m_slot.requestID() );
