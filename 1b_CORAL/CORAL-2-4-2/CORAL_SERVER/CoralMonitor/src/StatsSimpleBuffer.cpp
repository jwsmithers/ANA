...
namespace coral {
  ...
  runDatPlotter(void* p)
  {
    ...
    while(simplefp_active)
    {
      ...
      if (fd >= 0) // Fix Coverity RESOURCE_LEAK
      {
      ...
      for( std::map<std::string, StatsSimpleBufferPlot*>::const_iterator i = plots.begin(); i != plots.end(); ++i )
      {
        ...
        if(columns.size() > 0)
        {
          ...
          if (fd2 >= 0) // Fix Coverity RESOURCE_LEAK
          {
  ...
  StatsSimpleBuffer::StatsSimpleBuffer( size_t refresh_rate /* in seconds */,
                                        size_t maxsize )
    : m_databuffer()
    , m_plots()
    ...
    , m_dat_tid() // Fix Coverity UNINIT_CTOR
    , m_dat_attr()
  {
  }
